...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True, dir_okay=False),
    help="Path to a proposer-brief markdown file to copy into the epoch.",
)
@click.option(
    "--scoring",
    "scoring_source",
...
def new_cmd(
    ...
    cfg = lifecycle.new_epoch(
        ...
        # ``new_epoch``'s parameter is still named ``rubric_source`` —
        # the proposer-brief rename of the lifecycle module is a
        # separate workstream. The operator-facing flag is ``--brief``.
        rubric_source=Path(brief_source),
        weights=weights,
