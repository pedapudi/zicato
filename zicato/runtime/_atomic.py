...
Every state-file writer in :mod:`zicato.runtime` MUST go through
:func:`atomic_write_json` (or :func:`atomic_write_text` for non-JSON
payloads). The goal is a hard guarantee: no reader ever observes a
half-written file. A crash mid-write leaves the on-disk file either
untouched (at the previous content) or fully replaced with the new
