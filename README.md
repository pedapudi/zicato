...
- [`docs/design/DASHBOARD.md`](docs/design/DASHBOARD.md) — the live UI for an in-flight epoch: Starlette HTTP + SSE architecture, the L0→L4 drill-down (workspace → epoch → generation → round → run) with ⌘K palette and status pill, the full GET API surface, and the control-file protocol for operator actions.
...
- [`docs/design/ANALYTICAL-INDEX.md`](docs/design/ANALYTICAL-INDEX.md) — the `.zicato/index.db` SQLite analytical index: why cross-run views are queries not file-walks, the eight-table schema, the files-canonical / index-derived discipline, `zicato reindex`, and the nine-table schema (SCHEMA_VERSION 2).
