...
- [`docs/design/TUI.md`](docs/design/TUI.md) — `zicato tui`, the Console in the terminal: a keyboard-driven review surface over the *same* served payloads the browser dashboard renders (a second renderer, never a second brain — enforced by an import contract and a shared render cross-pin against `ui.js`), the six lenses, glyph microtypography (braille sparklines, shared-scale CI whiskers, champion-anchored margin bars, the round lifeline), the ported digest/repaint discipline, and the explicit render-conformance list of what stays in the browser.
