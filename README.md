...
- [`docs/design/RUNTIME.md`](docs/design/RUNTIME.md) — `.zicato/runtime/` state file layout, the Rust supervisor binary auto-spawned by `zicato evolve` (watchdog + dashboard server in one), heartbeat protocol, signal escalation, resume semantics on orchestrator restart, single-writer concurrency model.
- [`docs/design/DASHBOARD.md`](docs/design/DASHBOARD.md) — the live UI for an in-flight epoch: HTTP + SSE architecture, the eight panels (the Tournament view with the bracket, lineage, score trajectory, drift heatmap, loop-health panel, log tail, ...), v1.3 control-file protocol for operator actions, full API surface.
...
- [`docs/design/STORAGE.md`](docs/design/STORAGE.md) — v0 directory storage (shipping) and the v0+1 git-backed roadmap (G0-G10): one repo per workspace, one branch per epoch, tags per generation, worktrees for parallel tournaments, experiment metadata in commit messages, migration tooling, new operator commands (`zicato log` / `diff` / `show` / `bisect` / `blame`); the three-storage-concerns split.
- [`docs/design/ANALYTICAL-INDEX.md`](docs/design/ANALYTICAL-INDEX.md) — the `.zicato/index.db` SQLite analytical index: why cross-run views are queries not file-walks, the eight-table schema, the files-canonical / index-derived discipline, `zicato reindex`, and where SQLite is and is not used in zicato.
