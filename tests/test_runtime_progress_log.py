...
def test_append_returns_monotonic_gapfree_seq(tmp_path: Path) -> None:
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        with patch.object(writer.progress_log, "tail", wraps=writer.progress_log.tail) as scan:
            s1 = progress_log.append_progress(writer, progress_log.LOOP_START)
            s2 = progress_log.append_progress(writer, progress_log.ROUND_START)
            s3 = progress_log.append_progress(writer, progress_log.PROPOSE)
            s4 = progress_log.append_progress(writer, progress_log.TOURNAMENT_START)
...
def test_seq_advances_only_on_append_not_on_reread(tmp_path: Path) -> None:
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        ...
        progress_log.append_progress(writer, progress_log.PROPOSE)
...
def test_clear_log_resets_seq(tmp_path: Path) -> None:
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        ...
        assert progress_log.append_progress(writer, progress_log.LOOP_START) == 1


def test_clear_log_idempotent_on_absent(tmp_path: Path) -> None:
    """Clearing a never-written log is a no-op, not an error."""
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        progress_log.clear_log(writer)  # must not raise
    assert progress_log.tail_seq(tmp_path) == 0
