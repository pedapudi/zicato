...
def test_entry_with_input_only_is_single_turn() -> None:
    ...
    entry = Entry(id="e1", input="What is 2+2?", wall_clock_budget_seconds=60)
...
def test_entry_with_turns_list_of_strings_is_scripted() -> None:
    ...
    entry = Entry(
        id="e1", turns=["first user msg", "second user msg"], wall_clock_budget_seconds=120
    )
...
def test_entry_with_turns_already_scripted_turn_instances_is_scripted() -> None:
    ...
    entry = Entry(id="e1", turns=turns, wall_clock_budget_seconds=120)
    assert entry.kind == "multi_turn_scripted"
...
def test_entry_with_persona_is_emulated() -> None:
    ...
    entry = Entry(id="e1", persona=persona, wall_clock_budget_seconds=300)
...
# ---------------------------------------------------------------------------
# Entry: budget and Expectation pass-through
# ---------------------------------------------------------------------------


def test_entry_preserves_wall_clock_budget() -> None:
    """The builder preserves the declared wall-clock budget."""
    entry = Entry(id="e1", input="x", wall_clock_budget_seconds=42)
...
def test_entry_with_no_judges_defaults_empty() -> None:
    ...
    entry = Entry(id="e1", input="x", wall_clock_budget_seconds=30)
    assert entry.judges == ()
...
def test_entry_judges_attach_to_board_entry() -> None:
    ...
    entry = Entry(id="e1", input="x", judges=[j1, j2], wall_clock_budget_seconds=30)
...
def test_entry_judges_accept_tuple() -> None:
    ...
    entry = Entry(id="e1", input="x", judges=(j,), wall_clock_budget_seconds=30)
...
def test_entry_judges_reject_non_judge_spec() -> None:
    ...
    with pytest.raises(ValueError, match="JudgeSpec"):
        Entry(id="e1", input="x", judges=["not a judge"], wall_clock_budget_seconds=30)  # type: ignore[list-item]
...
def test_board_add_accepts_entry_factory_result() -> None:
    ...
    board = Board()
    board.add(Entry(id="e1", input="hi", wall_clock_budget_seconds=30))
...
def test_board_add_rejects_duplicate_id() -> None:
    ...
    board = Board()
    board.add(Entry(id="dup", input="x", wall_clock_budget_seconds=10))
    with pytest.raises(ValueError, match="already present"):
        board.add(Entry(id="dup", input="y", wall_clock_budget_seconds=10))


def test_board_add_returns_self_for_chaining() -> None:
    """Board.add returns the board so callers can chain."""
    board = Board()
    out = board.add(Entry(id="e1", input="x", wall_clock_budget_seconds=10)).add(
        Entry(id="e2", input="y", wall_clock_budget_seconds=10)
    )
    assert out is board
...
def test_board_save_load_round_trip(tmp_path: Path) -> None:
    ...
    board.add(Entry(id="scripted", turns=["a", "b"], wall_clock_budget_seconds=120))
    board.add(Entry(id="emulated", persona=persona, wall_clock_budget_seconds=200))
...
def test_board_disable_drift_round_trips(tmp_path: Path) -> None:
    ...
    board.add(Entry(id="e1", input="x", wall_clock_budget_seconds=30))
    path = tmp_path / "b.jsonl"
    board.save(path)
