...
def test_seed_variants_have_distinct_query_nodes_and_reflection_decisions(tmp_path: Path) -> None:
    ...
    assert refs == {
        "seed-none:v0:entry:tournament:r0",
        "seed-17:v0:entry:tournament:r0",
        "seed-23:v0:entry:tournament:r0",
    }
...
def test_exact_seed_resolution_survives_incomplete_alternate_publication(tmp_path: Path) -> None:
    alternate = _write(tmp_path, 31)
    selected = _write(tmp_path, 17)
    paths = WorkspacePaths(tmp_path)
    selected_id = run_id_for_unit("v0", "entry", base_seed=17, epoch_id="e0")
    assert (
        resolve_run_id_for_entry(
            paths, "e0", "v0", "entry", measurement=replace(TOURNAMENT_DRAW, base_seed=17)
        )
        == selected_id
    )
    expected_events = selected.with_name("events.tournament.r0.jsonl")
    assert (
    ...
    incomplete.unlink()
    missing_id = run_id_for_unit("v0", "entry", base_seed=None, epoch_id="e0")
    assert resolve_transcript_events(paths, "e0", "v0", "entry", run_id=missing_id) is None
    draws = cell_replicate_draws_indexed(paths, "e0", "v0", "entry")
    assert {profile.run_id for _, profile in draws} == {
        selected_id,
        run_id_for_unit("v0", "entry", base_seed=31, epoch_id="e0"),
    }
...
def test_index_audits_all_seeds_and_projects_only_selected_seed(tmp_path: Path) -> None:
    ...
    assert len(runs_for_generation(database, "e0", "v0")) == 3
    selected_id = run_id_for_unit("v0", "entry", base_seed=17, epoch_id="e0")
    ...
    ingest_run(tmp_path, database, "e0", "v0", "entry")
    assert [row["run_id"] for row in loss_profiles_for_generation(database, "e0", "v0")] == [
        run_id_for_unit("v0", "entry", base_seed=None, epoch_id="e0")
    ]
    assert len(runs_for_generation(database, "e0", "v0")) == 3
...
def test_index_refuses_conflicting_seed_record_without_replacing_projection(
    ...
    assert before == [(run_id_for_unit("v0", "entry", base_seed=17, epoch_id="e0"),)]
    raw = json.loads(path.read_text())
    if conflict == "base_seed":
        raw["measurement"]["base_seed"] = 23
    else:
        raw["run_id"] = run_id_for_unit("v0", "entry", base_seed=23, epoch_id="e0")
...
def test_interrupted_replacement_keeps_archive_visible_without_an_extra_draw(
    ...
    pending.mkdir()
    (pending / "loss.tournament.r0.json").write_text("{")
    layout = WorkspaceLayout.from_root(tmp_path)
    history = read_events_history(layout, "e0", "v0", "entry", base_seed=17)
    assert history == [[{"runId": run_id_for_unit("v0", "entry", base_seed=17, epoch_id="e0")}]]
    paths = WorkspacePaths(tmp_path)
...
def test_duplicate_physical_spelling_never_adds_an_independent_draw(tmp_path: Path) -> None:
    ...
    path = _write(tmp_path, 17)
    path.with_name("loss.tournament.r0.json").write_bytes(path.read_bytes())
    paths = WorkspacePaths(tmp_path)
    assert len(cell_replicate_draws_indexed(paths, "e0", "v0", "entry")) == 1
    corpus = ingest_lineage(
        workspace_root=tmp_path,
    ...
    assert audit == []
    assert path.with_name("loss.tournament.r0.json").read_bytes() == path.read_bytes()
...
def test_selected_conversation_uses_its_seed(tmp_path: Path, seed: int | None) -> None:
    ...
    paths = WorkspacePaths(tmp_path)
    run_id = run_id_for_unit("v0", "entry", base_seed=seed, epoch_id="e0")
    assert find_generation_run(paths, "v0", "entry") == (
        run_id,
        selected.with_name("events.tournament.r0.jsonl"),
    )
    assert find_generation_entry_events(paths, "v0", "entry") == selected.with_name(
...
def test_missing_exact_seed_has_no_outer_historical_fallback(tmp_path: Path) -> None:
    ...
    _write(tmp_path, 31)
    _write(tmp_path, 17)
    missing = run_id_for_unit("v0", "entry", base_seed=29, epoch_id="e0")
    assert (
...
def test_live_conversation_before_score_publication_requires_exact_identity(tmp_path: Path) -> None:
    ...
    assert resolve_conversation(
        paths,
        run_id_for_unit("v0", "entry", base_seed=17, epoch_id="e0"),
        gen="v0",
        entry="entry",
        epoch="e0",
    ) == selected.with_name("events.tournament.r0.jsonl")
