...
def record_gate_comparisons(root: Path) -> None:
    ...
    for epoch_id in list_epoch_ids(layout):
        ...
        for _, experiment in experiments:
            if experiment.parent_generation_id and experiment.outcome is not None:
                groups.setdefault(experiment.round_index, []).append(experiment)
        fields = {
            record.tournament_id: record.to_dict()
            for record in field_tournament_records(root, epoch_id)
        }
        for round_index, group in groups.items():
            if read_settlement_receipt(root, epoch_id, round_index) is not None:
                continue
            ...
            for experiment in group:
                ...
                gates.append(
                    {
                        **gate.explanation,
                        "champion": experiment.parent_generation_id,
                        "challenger": experiment.generation_id,
                        "parent_aggregate": parent,
