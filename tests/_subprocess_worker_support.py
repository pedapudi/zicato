...
class _CaptureBlockedSession(_CompletingSession):
    async def run(self, entry: Any, sinks: Any, config: Any) -> Any:
        ...
        capture.with_name(unit_artifact_name("result", index)).mkdir()
        return await super().run(entry, sinks, config)


class CaptureBlockedAdapter(CompletingAdapter):
    """Obstruct capture writes after the worker has archived prior artifacts."""

    def load(self, generation_root: Path) -> _CompletingSession:
        return _CaptureBlockedSession()


def make_capture_blocked_adapter() -> CaptureBlockedAdapter:
...
class StubAdapter:
    ...
    :func:`zicato.tournament.worker_transport.adapter_worker_spec` uses to make the
