...
def _bootstrap_swiss_workspace(
    ...
    **proposer: Any,
    ...
    Keyword arguments beyond those steer the stand-in proposal runtime
    (:func:`tests._foe_support.stand_in_proposer_block`), which is how a
    test whose subject is a misbehaving field — duplicate siblings, a
    challenger whose hypothesis will not validate, a proposer that can
    mint nothing — gets one.
    """
...
def test_a_round_spends_one_proposal_episode_per_candidate(
    ...
    """The spend a field costs: one episode per challenger, and no more.

    Every candidate is generated by one Foe episode, so what a round
    spends on proposing is countable — and it is counted here off the
    workspace's own durable record rather than off the stand-in that
    answered the episodes.
    """
    ...
    episodes = [r for r in read_proposer_inputs(workspace, epoch_id) if r["role"] == ROLE_PROPOSAL]
    assert [r["new_generation_id"] for r in episodes] == ["v1", "v2", "v3"]
    # Single-sample proposer: no slate, so no slot is recorded on any of them.
    assert {r["slot"] for r in episodes} == {None}

    # And each episode ran in its own process, registered under its own
    # candidate: the transcripts are filed one per challenger.
    episode_dir = workspace / "epochs" / epoch_id / "episodes"
    assert sorted(p.name for p in episode_dir.iterdir()) == ["v1", "v2", "v3"]
...
def test_field_diversity_soft_reject_persists_rejected_outcome(
    ...
    """
    # BOTH challengers state the SAME core idea over the same mutation
    # point: the second duplicates the first in-flight sibling and is
    # soft-rejected to keep the field diverse.
    workspace, epoch_id = _bootstrap_swiss_workspace(
        tmp_path, field_size=2, rounds_n=1, idea="swap the greeting string"
    )
    install_stub_adapter_factory(monkeypatch)
    install_telemetry_stubs(
        monkeypatch,
...
def test_field_status_when_all_challengers_rejected(
    ...
    # Every episode reports a block, so no challenger can be minted.
    workspace, epoch_id = _bootstrap_swiss_workspace(
        tmp_path, field_size=2, rounds_n=1, refuse="goal-unreachable"
    )
    install_stub_adapter_factory(monkeypatch)
    install_telemetry_stubs(
        monkeypatch,
...
def test_field_status_carries_per_attempt_validation_reason(
    ...
    ``drift:file_findability`` is neither a built-in drift kind nor a
    declared board judge, so a hypothesis predicting it is refused by the
    structured parser with exactly that message. Challenger v1 proposes
    normally and applies; challenger v2 predicts it and fails every
    attempt on the same error.
    """
    workspace, epoch_id = _bootstrap_swiss_workspace(
        tmp_path,
        field_size=2,
        rounds_n=1,
        hypotheses={"v2": {"predict": "drift:file_findability"}},
    )
    install_stub_adapter_factory(monkeypatch)
    install_telemetry_stubs(
        monkeypatch,
    ...
    assert by_gen["v1"]["hypothesis"] == "Tag the greeting literal for candidate v1."
    ...
    # One episode, one attempt: a proposal is an episode, and the repair
    # turns a bad hypothesis would get happen inside it rather than as
    # separate attempts on this side.
