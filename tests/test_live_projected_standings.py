...
def test_update_tournament_projected_merges_and_preserves(tmp_path: Path) -> None:
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        ...
        update_tournament_projected(
            writer, {"A": {"scalar": 2.0, "boards_done": 1, "boards_total": 4, "pass_rate": 1.0}}
        )
        update_tournament_projected(
            writer, {"B": {"scalar": 1.0, "boards_done": 1, "boards_total": 4, "pass_rate": 1.0}}
        )
...
def _racing_rung_state(writer: WorkspaceLock) -> None:
...
def test_projected_write_anti_flash_no_op_on_byte_identical(tmp_path: Path) -> None:
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        _racing_rung_state(writer)
        update_tournament_projected(
            writer, {"v1": {"scalar": 0.30001, "boards_done": 1, "boards_total": 2}}
        )
        ...
        update_tournament_projected(
            writer, {"v1": {"scalar": 0.30004, "boards_done": 1, "boards_total": 2}}
        )
...
def test_projected_write_converges_to_settled_when_rung_drops_live_progress(
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        _racing_rung_state(writer)
        update_tournament_projected(
            writer, {"v1": {"scalar": 0.3, "boards_done": 1, "boards_total": 2}}
        )
        ...
        write_active_tournament(writer, _replace(current, rounds=settled_rounds))
        ...
        update_tournament_projected(
            writer, {"v1": {"scalar": 0.25, "boards_done": 2, "boards_total": 2}}
        )
    back = read_active_tournament(tmp_path)
    assert back is not None
...
def test_partial_aggregate_and_projected_compose(tmp_path: Path) -> None:
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        ...
        update_tournament_partial_aggregate(writer, champion_agg={"scalar": 3.0})
        update_tournament_projected(
            writer, {"A": {"scalar": 3.0, "boards_done": 1, "boards_total": 2}}
        )
    back = read_active_tournament(tmp_path)
    assert back is not None
...
def test_incremental_scorer_writes_projected_per_board(tmp_path: Path) -> None:
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        ...
        scorer = _IncrementalScorer(
            weights, champion_id="v0", challenger_id="v1", board_total=3, writer=writer
        )
...
def test_incremental_scorer_no_ids_writes_no_projected(tmp_path: Path) -> None:
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        ...
        scorer = _IncrementalScorer(ScoringWeights(), writer=writer)
...
def _two_competitor_state(writer: WorkspaceLock, structure: str, standings: list[dict]) -> None:
    write_active_tournament(
        writer,
        ActiveTournament(
            tournament_id="t",
...
def test_overlay_swiss_does_not_project_copeland_points(tmp_path: Path) -> None:
    ...
    with acquire_workspace_lock(tmp_path, "test-publication") as writer:
        _two_competitor_state(writer, "swiss", standings)
        rounds = [
            {
                "round_index": 1,
                "matches": [
