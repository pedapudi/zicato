...
- **expected_metric_movements** — one or more predictions keyed by measured
  metric name, such as `drift:off_topic`, `judge:structure`, or
  `cost:tokens_spent`. Each names a direction and a `small`, `medium`, or
  `large` magnitude. Predict changes honestly, including regressions.
...
`expected_metric_movements` to make `hypothesis_match` look better.
