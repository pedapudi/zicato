...
| `zicato-design-proposer` | **Configure the proposing agent** — native and text capabilities, `proposer` inheritance, the generic `proposer_generate` / `proposer_review` stages, native Pi slate ownership, isolation from the target engine, and the fact that editing the proposer or a skill rolls the epoch like editing the brief. Registered with `register --proposer-path`. |
