...
> 1. **Shared mutable state behind per-instance artifacts is the recurring
>    bug class.** Whenever several logical things (replicates, slate
>    candidates, concurrent checkouts, re-derives) share one physical slot
...
|---|---|---|---|
| 1 | Replicate-cache clobbering (the worker wrote the canonical `loss.json`, which is replicate 0's slot) | `120f761` | yes, whenever `replicates > 1` |
| 2 | `git worktree prune` racing a concurrent `add` | `6b8f98e` | yes, under concurrent checkouts |
| 3 | A/A calibration false-zero floor (the replicate index was never stamped) | `6e96f22` | yes, for any stochastic harness |
| 4 | The client champion scan returned the first promoted generation rather than the reigning one | `54beb21` | yes (dashboard views) |
| 5 | An apparent evolve hang caused by the test suite's reaper killing a concurrent evolve's process group | `9ccbbba` | yes, on shared dev hosts |
| 6 | Best-of-N tree mismatch on the gauntlet path (one child slot re-derived per slate sample) | `7025a30` | yes (`best_of_n = 3` default) |
| 7 | The same slot bug on the field path, plus the diversity misjudgment | `7025a30` | yes, on multi-challenger structures |
| 8 | Evidence-gate replicate-slot reuse (replayed draws, clobbered replicate 0, one-sided sampling) | `eb55266` | scaffold-on (the gate ships enabled in scaffolded contracts) |
| 9 | A git `derive_generation` re-derive left a stale shared worktree | `7025a30` | yes (the git backend is the default store) |
| 10 | The contract hash embedded the cwd and the checkout path | `8d0a94f` | yes, in every relocated or differently-cwd'd checkout |
| 11 | `judge_view` opened the index in write mode on a read path | `8a46888` | yes (any index read while a run is writing) |
| 12 | The `elimFlow` client-side elimination model and its defensive guards | `1621ebe` | yes (elimination structures in the console) |

---

## Case 1 — Replicate-cache clobbering: the worker's canonical `loss.json` is replicate 0's slot
...
The bug produced no visible symptom. Replicated matchups (`replicates > 1`)
...
replicate 0 of each unit held some other replicate's draw.
...
replicate 0's immutable cache slot. Nothing routed the two apart, so the
cache slot ended up holding whatever the last writer produced.
...
- The in-memory aggregation used the losses returned from the runs rather
  than the files, so scores were correct *within* the round; only later readers
...
Replicate 0 still maps to the canonical path, so every single-replicate path
writes byte-identical output, and r>0 lands in its own `loss.r<r>.json`.
...
`tests/test_decision_procedure_power.py` asserts this replicate-independence
property). Against the buggy code both reads return the last draw and the
assertion fails. The oracle needs σ>0: at σ=0 the draws are equal by value and
...
The failure was rare, timing-dependent, and never reproduced when the
checkouts ran one after another.
...
The docstring on the buggy code asserted the opposite — that `worktree add`
"holds an 'initializing' lock … so a concurrent sibling's prune can never race
a half-created checkout." That claim was wrong in practice: the race was
...
- The conformance suite exercised checkouts one at a time. The race only
  surfaced when an 8-way concurrent conformance test was written for the
  snapshot-retention work — widening the test topology found the bug, rather
  than a field report.
...
the next prune-before-add. The commit also records the alternative that was
benchmarked and rejected: a per-run `git archive` plus tarfile, which has no
shared admin state at all, but costs 1.5–2.5× as much serially and serializes
catastrophically on the Python side under threads. A later reader who wants to
remove the shared state can see there why that alternative lost.
...
statistically across CI runs. Because it is timing-dependent, it has to live
in the permanent suite rather than be verified once by hand.
...
- you build any other multi-command sequence over shared git state (branch
  ...
  lock." It locks each command rather than your protocol. See also the stale
...
which gave each draw its own slot, but never **stamped** the index onto the
board entries — and the stamped context is the only channel a seeded harness
derives its noise draw from:
...
scalars with zero spread. The cache stored K copies of one sample in K
distinct slots.

### Why every oracle missed it

- The calibration's own end-to-end (e2e) test used the **deterministic**
  target_0 adapter, whose *correct* floor is exactly 0.0. The test asserted the
  buggy value as the right answer, because for that harness it *was* the right
  answer; the bug and the pass condition coincided.
- The seeded-noise adapter that could tell a quiet harness from broken seeding
  was built in a separate commit (`3cbb637`), and no test yet drove the
  calibration through it. The bug surfaced only when the pre-flight work
  composed the two.

### The invariant class it defines

**The cache key and the harness stamp must be the same number, set at the same
seam.** More generally: when a parameter has two consumers, persistence keying
and behavior seeding, passing it to one and not the other produces a system
that *stores* correctly and *measures* wrongly. That is the worst combination,
...
Stamp the board the same way the replicated-duel path does, from one local
variable, so key and stamp cannot diverge:
...
## Case 4 — The client champion scan returned the first promoted generation instead of the reigning one
...
Dashboard views (generations table, epoch view, candidate view, boards, shell)
could crown the **wrong generation** with the champion glyph, and key their
...
`gens.find((g) => g.promoted)` answers "some promoted generation" rather than
"the end of the promoted spine." The correct derivation — walk the promoted
parent→child chain to its last element — existed server-side in the lineage
readers, but the client carried its own second implementation of the decision,
one per view, each of them slightly different. The fix commit's message
generalizes: "The frontend re-implemented decisions the server already owns —
and could disagree with it."

### Why every oracle missed it

- Most test epochs, and most demo runs, contain **a single promotion**: the
  first promoted generation and the reigning one coincide, so every scan
  returns the right answer on the fixtures.
...
every consumer reads the stamp verbatim. A consumer that re-derives the value,
especially one that parses display strings or scans raw arrays, is a second
implementation of the same decision, and the two will diverge. Secondary
lesson: **"find first" over an unordered collection is a warning sign**
whenever the domain concept is the latest or last element of a chain.
...
## Case 5 — The apparent evolve hang was the test suite's reaper killing a concurrent evolve's process group
...
round, while `--no-dashboard` ran fine. The exit status told a different
story: the process died with rc=137 (SIGKILL) within about a second of
startup, and the log ended right after the supervisor's port-bind line, which
read as a startup hang. The harmonograf launch path kept working, which made
the misreading easier to hold.

### Root cause

Two mechanisms, each reasonable on its own, combined to kill the run:
...
evolve invocation* sharing that group. The standalone `zicato dashboard` has
different argv and no `-m zicato.dashboard` marker, so the reaper never
selected it, which is why the harmonograf launch path was unaffected.
...
- The symptom pointed at the process that died rather than the one that
  killed it, so debugging started at evolve's startup.
...
- **Blast-radius isolation:** a child that third parties might signal by
  group must be its own session and process-group leader, so group-directed
  signals reach that child's subtree and nothing else.
- **Provenance before force:** a safety net that kills processes must select
  ...
  temp root). Selecting by appearance time and argv shape classifies unrelated
  processes as leaks.
...
group-kill of the dashboard takes down the dashboard subtree and nothing else;
explicit teardown signals the child pid directly and is unaffected). The
conftest reaper selects only dashboards whose `--workspace` argv lies inside
this session's pytest temp root, and never signals its own process group
(bare-pid fallback).
...
foreign-workspace dashboard is never signalled; a same-group leak is killed by
bare pid rather than by `killpg`. The test does more than assert
`start_new_session=True` in the code, which is an implementation detail: it
fires the signal and checks which process survived.
...
- **Deterministic contracts pinned `best_of_n = 1`.** With a single sample the
  chosen candidate is by definition the last validated one, so the mismatch
  cannot occur. The convergence oracle, every end-to-end test, and every gate
  test ran with the interacting knob OFF; this is the clearest instance of the
  pinned-knob lesson below.
...
derived from the experiment persisted for it. More generally — when a
...
The first form of that re-derive (`_align_child_tree`) was CONDITIONAL: it
fired only when the chosen candidate was not the last-validated one, and an
...
invariant is the same and no branch can skip the derive.

### The regression test pattern that pins it

Make the chosen candidate something other than the last one, then check the
mounted *tree* rather than the returned object.
`tests/test_best_of_n_tree_integrity.py` scripts a 3-candidate slate where the
critic picks candidate 0 and asserts that the mounted tree contains candidate
0's patch. It does so through the full known-answer e2e over target_0 with real
subprocess workers, for BOTH pipelines. The wrapper-seam unit tests in
`tests/test_proposer_best_of_n.py` pin the mount ladder (an earlier pick is
re-derived, the last pick is mounted too, a hookless context is left untouched,
a mount failure raises without attempting a fallback, a raising hook is folded
into that error).
...
- you optimize away the post-selection re-derive (`_mount_chosen`) because
  ...
  and the scratch tree the candidate validated into is discarded;
...
**Fix commit:** `7025a30` — the same commit as the gauntlet tree-mismatch case
(case 6). It is a separate case here because it breaks a different mechanism
and its oracle gap has a different shape.

### Symptom

On multi-challenger structures (racing, swiss, elimination), the same
tree-versus-experiment mismatch as the gauntlet case (case 6), and one more:
the **field-diversity constraint judged the wrong hypothesis**. The diversity
gate compares the chosen candidate's hypothesis signature against the
already-minted field to soft-reject near-duplicates; it judged the CHOSEN
candidate's signature while the tree that actually entered the field carried
the LAST-SAMPLED candidate's edits. A field could pass diversity screening
while being behaviorally homogeneous — or be rejected as duplicate while
behaviorally novel.
...
Everything from the gauntlet case (case 6), plus one gap of its own. The
diversity gate is opt-in and soft, emitting a soft-reject signal, and its tests
fed it hypothesis signatures directly rather than through the mount path.
Agreement between signature and tree was therefore untestable as specified, and
no assertion anywhere said that the signature judged belongs to the tree
fielded.

### The invariant class it defines

**Derived judgments inherit the identity bug of their inputs.** When record and
artifact can disagree, which is the class the gauntlet case (case 6) defines,
every judgment computed from the record (diversity, parsimony diff-size,
exemplar attribution) silently becomes a judgment about the wrong artifact.
Fixing the slot fixes these *by construction* — which is why the fix commit
repaired "all three symptoms" at one seam rather than patching the diversity
call site.

### The fix

The same post-selection re-derive as the gauntlet case (case 6), through
`_mount_chosen`: once the mounted tree is the chosen candidate's tree, the
diversity check is correct by construction. The diversity code itself does not
change; the fix sits upstream, at the identity seam.

### The regression test pattern that pins it

The field-pipeline e2e in `tests/test_best_of_n_tree_integrity.py` asserts, for
the racing field, the mounted and committed tree, the persisted experiment, the
known scalars, **and the field-diversity signature agreement**. That last
assertion is the one this case adds over the gauntlet case (case 6). Pattern to
reuse: when a bug corrupts an *identity*, enumerate every derived judgment
downstream of that identity and pin each one, rather than only the first
artifact.
...
Nothing visible, and what was observable looked reassuring: the Bradley–Terry
pre-gate's deferred crownings **resolved faster than they should have**. In
fast mode, confidence intervals separated after a modest budget, on data that
...
1. **Fast mode — replay:** the child cache-read its canonical `loss.json`,
   ...
   alone, so the intervals "separate" with no new evidence. That is an
   unsound promotion produced by the check that exists to prevent unsound
   promotions, and the scaffolded contracts ship the gate ON.
2. **Full mode — clobber:** `force_fresh` re-ran at slot 0 and re-persisted
   ...
   key on — the slot of the replicate-cache clobbering case (case 1),
   corrupted here by a different writer.
...
- The e2e that covered the loop (`tests/test_gauntlet_evidence_gate_e2e.py`)
  ...
  *by value*, and the test as written documented the replay approvingly as "a
  pure cache read." The deterministic oracle recorded the bug as an
  optimization.
...
- **Independence is a property of the wiring rather than of the intent.**
...
- `EVIDENCE_REPLICATE_BASE = 4000` joins the reserved ladder, and a
  ...
  evidence replicate j at `4000+j`, drawing BOTH sides fresh on a natural
  cache MISS (which stays idempotent on a resumed confirm).
- Matchup ids encode the slot (`bt-replicate:r{slot}:{left}:{right}`).
- The driver's confirm loop refuses to append a result whose matchup id
  already appears in the audit, while still counting the spend; a duplicate
  that did not count would loop forever.
- The field replicate stops overwriting `gen_score.json` with a single-draw
...
**Fix commit:** `7025a30` (the "Also fix" paragraph). The repair of the
gauntlet tree-mismatch case (case 6) exposed it, by making re-derives of one
child id a hot path.

### Symptom

Three operations re-derive an already-materialised child generation: a proposer
retry after failed post-apply validation, the best-of-N chosen-candidate
re-derive (case 6), and a crash-resume re-validate. After any of them, **direct
readers of `snapshot_root` saw the OLD attempt's tree**, a tree that did not
match the generation's commit. Workers, which mount the tag via ephemeral
checkout, saw the new content; in-process readers of the shared worktree saw
the stale one. Two views of one generation, silently different.
...
- Re-derives of the same child id were rare until the gauntlet
  tree-mismatch fix (case 6) made them a designed operation. The bug stayed
  latent until that adjacent repair moved the code path from error recovery to
  every best-of-N round.
...
bug. It is the shared-registry mistake of the worktree-prune race (case 2) in
single-threaded form: two representations and one update, with no race
required.
...
- The golden could not catch it alone either: a golden captured in checkout A
  ...
  re-capture" unless someone asks *why* the bytes differ. The parity gate
  caught this only because someone read the red golden as information rather
  than as a chore.
...
side. The content-digest fallback identity the target_0 harness gives an
ephemeral snapshot is another. Corollary: challenge any `Path.resolve()` that
appears in a hash, key, or identity computation during review.

### The fix

Normalize the strings and never resolve them: `.`, `..`, and separator
spelling are collapsed, the result is POSIX-rendered and sorted, and the whole
computation touches the filesystem nowhere:
...
(recorded in the changelog, with the standard contract-roll behavior) rather
than as a compatibility shim: the hash was wrong, and keeping it stable would
have preserved the error.
...
- you fix a red golden by re-capturing it without first explaining the byte
  diff (see 11-testing.md §11.7.8, the never-bake-a-sibling-change rule);
...
`judge_view.py` carried two ad-hoc `sqlite3.connect(...)` calls, and one of
them opened the index in the default read-write mode. A read handler holding a
writable connection does two harmful things. It contends for the write lock
with the live ingester, which produces the `database is locked` errors. It can
also create the `-wal` and `-shm` sidecars and a rollback journal on a database
the reader has no business mutating, which perturbs the reindex-dump golden
captured from that workspace. A reader that can write can change the database
it is reporting on.

### The fix

Both connects route through the shared `open_index_ro` context manager
(`query/_sqlite.py`), which opens the index at the URI `file:<path>?mode=ro`
and guarantees the close. Every reader in `query/` uses that one lifecycle. A
reader cannot take the write lock or create a journal.
...
- you add a defensive guard to a figure builder against a "malformed" backend
  payload. That guard is a sign the payload is under-specified: fix the
  SERVER'S model rather than growing a client-side patch;
...
Twelve bugs, three recurring shapes. These three lessons generalize across
every case above.

### M1 — the shared-mutable-state lesson: many logical artifacts on one physical slot

Six shared slots produced seven of the cases above:

- one `loss.json` serving as both worker output and replicate 0's cache slot
  — the replicate-cache clobbering case (case 1);
- one worktree admin registry under concurrent multi-command windows — the
  worktree-prune race (case 2);
- one child-snapshot slot under N slate candidates — the best-of-N tree
  mismatch on the gauntlet and field paths (cases 6 and 7);
- one materialised worktree under a moving tag — the stale shared worktree
  (case 9);
- one canonical slot under evidence "replicates" — the evidence-gate slot
  reuse (case 8);
- one process group under two owners' signals — the test reaper (case 5).

**Seven of the twelve cases are the same bug** at six different addresses:
several logical artifacts multiplexed onto one physical resource, under
last-writer-wins semantics nobody chose.
...
If the answer involves the word "reuse," write down who invalidates the shared
slot and when; otherwise the design will produce another case like these.

### M2 — the pinned-knob lesson: bugs hide behind knobs the test contracts pin off

The best-of-N tree mismatch (cases 6 and 7) hid behind `best_of_n = 1` in every
deterministic contract. The evidence-gate slot reuse (case 8) hid behind σ=0,
where a replay equals a fresh draw by value, and the end-to-end test described
the replay approvingly. The calibration false-zero floor (case 3) hid behind a
deterministic harness whose correct answer equalled the bug's output. The
replicate-cache clobbering (case 1) needed both `replicates > 1` AND σ>0 before
it could be observed. The pattern is the same in each: the oracle's world is
configured so that the interaction that breaks cannot occur, and the green
suite certifies the configuration rather than the code.
...
evidence), at least one test must run with both engaged. That test needs a
world where their interaction has observable consequences: seeded noise,
multi-candidate slates, multi-promotion spines, or concurrent checkouts. When
you add a knob, ask which existing mechanisms it interacts with, and where the
test that runs both lives; "does the knob work?" is the smaller question. The
...
slot-integrity test compares the bytes on disk; the champion test uses a spine
where the first promoted generation differs from the last; the hash test
computes from two real cwds. A regression test that passes both with and
without the fix pins nothing, and it will stay green when the bug returns.
...
refactors and keep failing for the right reason. Second, a fix's honest
numbers sometimes move an existing pinned expectation; the evidence-gate slot
reuse (case 8) changed the end-to-end test's budget expectations. Update that
expectation **in the fix commit, with the explanation inline**. A silently
adjusted pin is indistinguishable from a fudged one.
...
1. Which physical slots does my change write, and how many logical instances
   map onto each? (the shared-mutable-state lesson)
...
5. Does any consumer of my change re-derive a truth some owner already
   stamps? (the client champion-scan case, case 4)
6. If my change kills, sweeps, or cleans anything: can it *prove* ownership
   of every process or file it acts on? (the test reaper case, case 5)

Cross-references. The statistical machinery these bugs corrupted is
04-evaluation-statistics.md. The storage and worktree model behind the
worktree-prune race (case 2) and the stale shared worktree (case 9) is
07-runtime-and-durability.md §7.4. The testing discipline — oracles, goldens,
monkeypatch anchors — is 11-testing.md. How new work gets proposed, so that it
does not become the next case here, is 14-goals-and-roadmap.md §5, "How to
propose new work".
