...
| Patch targets a forbidden / stale mutation id | `check_patch_manifest_and_forbidden` | `RuntimeError` propagates — this is a hard programming/contract error, not a retryable | none beyond the raise | the hypothesis's `modulating` set is the ONLY thing patches may touch |
