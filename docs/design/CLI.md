...
> command was added and then descoped to three lenses; everything else
> unchanged since the 2026-08-09 pass). Verified the
...
measured signal is at/below the floor; WARN when every probe scored
...
not zero — pick a representative point); OK otherwise. Also places
promote_margin against the floor and the measured signal and names the side
it fell outside of — WARNING-class only, since the upper number is
DEGRADATION headroom and does not bound what a challenger can improve
(issue #119). Recommend-only — never gates. The verdict persists onto the
epoch record and flows into the per-round health report.
...
surface, rendering the *same* served payloads. Three lenses (Home, Standings,
Instrument) with `1`–`3` to jump, `j`/`k` to move, `enter` to drill, `b` to go
back, and `?` for help; Candidate, Board and Health are designed and deferred.
**Read-only**: the console mutates nothing and runs nothing — the Instrument
lens prints the exact `zicato reflect apply …` line for the operator to run.
Needs the `tui` extra (`uv sync --extra tui`); without it the command exits
with an install instruction. See [`TUI.md`](TUI.md).
...
| `--view TEXT` | unset ⇒ Home | Open a lens directly. Takes the SAME path the browser's hash router takes — `/e/<epoch>/gens`, `/e/<epoch>/instrument` — or a shorthand like `standings` or `instrument`. An address for a deferred lens still resolves, to the nearest one that exists. |
