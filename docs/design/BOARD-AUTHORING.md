...
| `tags=` | Operator labels for pattern slicing. `holdout` and `facet:{name}` are RESERVED — see BOARD-FORMAT.md §1.4 for what each does. A `facet:` tag is display-only: the dashboard groups the entry into that slice and nothing else changes. Default `()`. |
