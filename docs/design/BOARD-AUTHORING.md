...
| `tags=` | Operator labels for pattern slicing. Tags prefixed with `facet:` add the entry to a named diagnostic slice, and each generation records a mean per slice beside its scalar (BOARD-FORMAT.md §1.4). Default `()`. |
