...
| Rating (Bradley–Terry) layer *under* these structures — SHIPPED as the opt-in `params["rating"]` θ-rank standings + the visibility Elo fold; winner-resolution (Ranked Pairs, maximal lotteries) remains FUTURE / not implemented | [`SELECTION-THEORY.md`](SELECTION-THEORY.md) |
