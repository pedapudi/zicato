...
- Two items are **alarm-only in v1** and worth tracking as future hardening: diff-containment violations and the promotion-gate contradiction check *record* findings but do not *block* a promotion. Turning either into a hard block is a policy decision, not new mechanism.
...
2. **Retire the designed-only delta materialization** (`GENERATION-ISOLATION.md`) — the git default's blob dedup + worktree isolation obsoletes it; don't build a third mechanism.
