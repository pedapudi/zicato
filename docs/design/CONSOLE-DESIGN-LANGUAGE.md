...
| `metaLoopLedger` | the **cross-epoch home overview** — one composed figure braiding a **held-floor staircase** (best scalar each contract held), **effort-proportional epoch bands** (band width ∝ generations spent), and a **contract-component heatstrip** (which lever moved at each reset — board · brief · scoring · entrypoint · mutable_trees · structure · **proposer**, the last a column the plain diff omits). A structure roll is a SOFT seam (cross-roll floors are not comparable) — dashed on the staircase + that cell. Digest-gated (`metaLoopLedgerDigest`); degrades on 0–1 epochs. Answers "is the meta-loop making net progress across contracts, which lever moved each reset, is effort buying floor." |
