...
| **Telemetry insights** | LLM-summarised per-round observations (`ProposalEvidence.insights`, rendered by `render_evidence`) | Possibly (free text — may name entries) | **Medium.** An auxiliary LLM summary that can re-surface per-entry specifics the structured channels withheld. |
