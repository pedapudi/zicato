...
- A frozen system-under-test identity — the validated adapter worker document,
...
- The regression baseline rebases (a major refactor of the system
  under test happened outside the loop and the parent `v0` of the next
...
workspace data while any prose the evaluation LLM has already written is
...
The system under test's source content inside the registered mutable trees is not
