...
| 11 | `runtime/active_tournament.json` | `runtime/state.py::ActiveTournament` decodes the snapshot; `runtime/tournament_log.py` folds the authoritative live event stream. | Existing protocol; audit open | Existing protocol: keep snapshot fallback explicitly subordinate to the event stream. Audit malformed envelope and field coercion policy without adding a competing writer. |
...
| 23 | `epochs/<epoch>/mutations.json` | `core/mutation.py::MutationPoint` declares facts; `evolve/round_baseline.py` writes dictionaries, while query mutation/epoch views parse them independently. | Open | Open: one mutation-inventory codec for source coordinates and marker facts. Keep the containment attestation separate: it binds source bytes and does not replace this inventory. |
...
| 25 | `epochs/<epoch>/contract_components.json` | `epoch/contract.py` produces component hashes; epoching and `query/events_index.py::_read_contract_components` interpret the stored mapping independently. | Open | Open: shared hash-mapping acceptance and atomic publication. The query stringifies arbitrary keys/values. Frozen byte capture and publication intents do not themselves remove these readers. |
...
| 41 | Reflection `findings.json` | `reflection/findings.py` owns finding facts and the ranked collection, strict inverse decoding, reading, and durable publication. | Implemented extraction | Complete for this slice: query, index, CLI report, and draft application share acceptance. A malformed member refuses the collection before index changes or draft creation. Reading checks operation structure without invoking operations; their semantic validation stays with emission and application. |
...
| 43 | Reflection `suggestions.json` | `reflection/suggestions.py` owns strict typed decoding, collection identity checks, ranked atomic publication, and the sole reader. | Implemented extraction | Complete for this codec slice: raw reading is removed; builder inbox, apply, report, and trace views share acceptance. Malformed members refuse the collection. Existing list APIs retain empty results for absence and valid empty collections. Builder and trace views display server-owned unreadable reasons, including reason changes between refreshes. |
| 44 | `epochs/<epoch>/proposer_reflections/<id>/findings.json` | `proposer/reflection_records.py` owns the existing reflection, finding, and remedy types, strict inverse codecs, location validation, reading, and durable publication. | Implemented extraction | Complete for this slice: pending queue, query, CLI preview, and apply share acceptance. Decoding validates redaction and remedy text integrity without executing operations. Typed edits preserve extension fields and historical number types. Investigation remains a redacted recorded snapshot; no scorecard is recomputed while reading. The index does not consume this record. |
