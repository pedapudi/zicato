...
def proposal_runtime(ctx: CheckContext) -> Iterator[Defect]:
    """The workspace must be able to open a proposal episode.

    Every candidate is generated by one Foe episode, so a workspace that
    cannot start one cannot run a round at all. Three things are provable
    here without spending anything, and each fails a round later if it is
    not caught now:

    * a ``proposer`` block is declared, or a class of the operator's own
      is bound — otherwise the workspace has not said how it proposes;
    * the block parses and every field is in range, including the retired
      configurations that are refused by name;
    * the binary it names is an absolute path that exists and is
      executable. Foe searches no path, because an episode's grants are
      absolute and a relative spelling would mean different things to the
      loop and to a worker; a freshly initialized workspace therefore
      carries a placeholder, and this is where an operator is told to
      replace it.

    What is NOT proved is that the binary runs, that its build matches the
    pinned package, or that the model credential is accepted. The first
    two are settled when the epoch's contract hash is computed, which asks
    the binary for its fingerprint; the third needs a round trip this gate
    does not make.
    """
    from zicato.proposer.external import (  # noqa: PLC0415
        PROPOSER_AGENT_KEY,
        PROPOSER_BLOCK_KEY,
        UNSET_BINARY,
    )
    ...
    if str(runtime.get(PROPOSER_AGENT_KEY) or ""):
        # An operator's own class owns its own configuration; the seam
        # cannot know what would make it runnable.
        return

    block = raw.get(PROPOSER_BLOCK_KEY)
    if isinstance(block, dict) and str(block.get("binary") or "") == UNSET_BINARY:
        yield (
            "proposal_runtime_binary_unset",
            f"proposer.binary is still the scaffold's placeholder {UNSET_BINARY!r}; "
            "name the absolute path of the Foe binary this workspace runs its "
            "proposal episodes with",
            {"binary": UNSET_BINARY},
        )
        return

    try:
        config = load_foe_proposer_config(raw, ctx.workspace_root)
    except ProposerConfigError as exc:
        yield ("proposal_runtime_unusable", str(exc), {})
        return

    binary = config.binary
    if not binary.is_file():
        yield (
            "proposal_runtime_binary_absent",
            f"proposer.binary names {binary}, which is not a file",
            {"binary": str(binary)},
        )
        return
    if not os.access(binary, os.X_OK):
        yield (
            "proposal_runtime_binary_not_executable",
            f"proposer.binary names {binary}, which is not executable",
            {"binary": str(binary)},
        )
