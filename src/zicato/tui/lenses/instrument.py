...
The TUI never applies anything itself. An apply shells to the same command an
operator would type, so the audit trail is identical whether the operator typed
it or pressed ``a`` — there is no privileged mutation path behind this surface.
    ...
    for f in findings:
        ...
        rows.append(
            row(
                ...
                action=_apply_command(f, reflection_id),
                selectable=True,
            )
        )
        if f.get("detail"):
...
def _apply_command(f: dict[str, Any], reflection_id: str) -> str | None:
    """The CLI invocation that applies this finding — the only apply path."""
...
def _queue_block(findings: list[dict[str, Any]], reflection_id: str) -> Block:
    ...
    return Block(
        ...
        rows=tuple(rows),
        note="press a to run the apply command — it shells out; nothing is applied in-process",
    )
