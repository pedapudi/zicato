...
def _loop_block(traj: dict[str, Any], cost: dict[str, Any], ctx: LensContext) -> Block:
    ...
    rows.append(
        kv_row(
            "cost",
            "cost per promotion",
            present.cost_per_promotion_label(cost) or present.NULL,
        )
    )
    rows.append(kv_row("movement", "recent movement", present.fmt(movement, 4)))
...
def _floor_text(floor: dict[str, Any]) -> str:
    """The A/A floor as ``±0.0123 · 6 runs``, or the null glyph when unmeasured."""
...
def _champion_block(epoch: dict[str, Any], lineage: list[Any], ctx: LensContext) -> Block:
    ...
    rows = [
        row(
            ...
            action=f"candidate/{champion_id}",
            selectable=True,
        ),
        row(
            "champion-rating",
