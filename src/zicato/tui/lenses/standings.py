...
def _gauntlet(
    ...
    cells = [["challenger", "vs champion", "Δscalar", "decision", "rating"]] + [
        [
    ...
    for m, cell in zip(matchups, cells[1:], strict=True):
        ...
        rows.append(
            row(
                ...
                (pad(cell[3], widths[3]), DECISION_STYLE.get(decision, "plain")),
    ...
    note = "Δscalar is challenger − champion on the loss axis: negative is better"
    digest = [
        [
            m.get("challenger"),
            m.get("champion"),
