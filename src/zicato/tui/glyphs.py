...
* :func:`margin_bar` — a signed, champion-anchored block-element bar with the
  promote-margin threshold marked.
...
def margin_bar(
    ...
    *,
    ...
    """Render a signed margin as a champion-anchored bar.

    The bar grows RIGHT from a fixed anchor for a positive margin (challenger
    ahead of the champion) and LEFT for a negative one, so sign is read from
    direction, not from a minus sign that a narrow column might clip. Unicode
    uses eighth-blocks for sub-cell resolution; ASCII uses whole ``#`` cells.
    ``threshold`` (the promote margin) is marked with ``┆``/``:`` — a bar
    that has not reached its mark is visibly short of it.

    ``scale`` is the value at full deflection. Values beyond it are clamped and
    flagged with ``›``/``>`` at the edge, so a clamp is never silent.
    """
    half = max(width // 2, 1)
    num = _finite(value)
    if num is None or not math.isfinite(scale) or scale <= 0:
        return NULL.center(half * 2)

    cells = [" "] * (half * 2)
    frac = min(1.0, abs(num) / scale)
    clamped = abs(num) > scale
    filled = frac * half
    full = int(filled)
    rem = filled - full

    if num >= 0:
        for i in range(min(full, half)):
            cells[half + i] = "#" if ascii_only else _BLOCKS[-1]
        if not ascii_only and full < half and rem > 0.0625:
            cells[half + full] = _BLOCKS[min(7, int(rem * 8))]
    else:
        for i in range(min(full, half)):
            cells[half - 1 - i] = "#" if ascii_only else _BLOCKS[-1]
        if not ascii_only and full < half and rem > 0.0625:
            cells[half - 1 - full] = _BLOCKS[min(7, int(rem * 8))]

    if threshold is not None:
        t = _finite(threshold)
        if t is not None and 0 < t <= scale:
            at = half + min(half - 1, int(t / scale * half))
            if cells[at] == " ":
                cells[at] = ":" if ascii_only else "┆"
    if clamped:
        edge = (half * 2 - 1) if num >= 0 else 0
        cells[edge] = ">" if ascii_only else "›"
    return "".join(cells)
...
__all__ = ["NULL", "STAGES", "lifeline", "margin_bar", "sparkline", "whisker"]
