// variants/J/views/home.js — HOME / ENVIRONMENT: the workspace as a fleet.
//
...
export async function render(host, ctx) {
  if (!host.firstChild) host.appendChild(el('p', { class: 'dj-empty', text: 'Acquiring fleet telemetry…' }));
  ...
  gatedSwap(host, digest, () => {
    ...
    nodes.push(el('div', { class: 'dj-pagehead' }, [
      el('h1', { class: 'dj-h1', text: 'Environment' }),
      el('p', { class: 'dj-lede', text: 'The workspace as a fleet — every epoch at a glance. Lower scalar (loss) is better.' }),
    ]));
    ...
    const fleet = rows.length === 0
      ...
      : el('div', { class: 'dj-fleet' }, rows.map((r) => fleetCard(r, r.epoch_id === current, ctx)));
    ...
    if (trendVals.length >= 2) {
      ...
      card.appendChild(el('p', { class: 'dj-faint', style: 'font-size:11px;margin:8px 0 0;', text: 'best scalar per epoch · lower is better' }));
...
function overviewStrip(rows, live) {
  ...
  return el('div', { class: 'dj-panel dj-row dj-overview' }, [
...
function statTile(value, key, foot) {
  return el('div', { class: 'dj-tile' }, [
    el('span', { class: 'dj-tile-value', text: value }),
    el('span', { class: 'dj-tile-key', text: key }),
    foot ? el('span', { class: 'dj-tile-foot', text: foot }) : null,
  ].filter(Boolean));
}
...
function fleetCard(row, isCurrent, ctx) {
  ...
  const head = el('div', { class: 'dj-fleet-head' }, [
    el('span', { class: 'dj-fleet-id', text: row.epoch_id }),
    el('span', { class: 'dj-chip dj-chip-' + (liveHere ? 'live' : st), text: liveHere ? 'running' : st }),
  ]);
  const goal = el('div', { class: 'dj-fleet-goal', text: row.goal || '(no goal recorded)' });
  const hero = el('div', { class: 'dj-fleet-spark' }, [
    sparkVals.length >= 2
      ? svg.sparkline({ width: 240, height: 46, values: sparkVals, band: true, goodDirection: 'down' })
      : el('span', { class: 'dj-faint', text: 'no trajectory yet' }),
  ]);
  const stats = el('div', { class: 'dj-fleet-stats' }, [
  ...
  return el('a', {
    class: 'dj-fleet-card' + (isCurrent ? ' dj-is-current' : ''),
    href: ctx.href('epoch', { epochId: row.epoch_id }),
  }, [head, goal, hero, stats]);
}

function miniStat(k, v, tone) {
  return el('div', { class: 'dj-mini' }, [
    el('span', { class: 'dj-mini-k', text: k }),
    el('span', { class: 'dj-mini-v' + (tone ? ' dj-good-t' : ''), text: v }),
  ]);
}
...
function healthPanel(hr) {
  ...
  const body = el('div');
  if (healthy) {
    body.appendChild(el('div', { class: 'dj-good-t', text: '✓ loop is healthy — the evaluation distinguishes candidates.' }));
  } else {
    for (const f of findings) {
      const sev = String((f && (f.severity || f.level)) || 'info').toLowerCase();
      body.appendChild(el('div', { class: 'dj-finding' }, [
        el('span', { class: 'dj-chip dj-chip-' + (sev === 'critical' ? 'closed' : 'open'), text: sev }),
        el('span', { class: 'dj-mono', style: 'margin-left:8px', text: f.detector || f.name || 'finding' }),
        el('div', { class: 'dj-faint', style: 'margin-top:4px', text: f.summary || f.message || '' }),
      ]));
    }
  }
  return section('Loop health' + (hr.epoch_id ? ' · ' + hr.epoch_id : ''), el('div', { class: 'dj-panel' }, [body]));
}
