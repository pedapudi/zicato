// variants/J/views/epoch.js — EPOCH: the dense data substrate of one epoch.
//
// Console's epoch screen leads with the OBJECTIVE, the collapsible proposer
// brief, then the figures: a non-colliding lineage BUMPS chart, the board
// entries × generations drift-loss HEATMAP (theme-aware, reads in all three
// themes), and a tight board TRELLIS of small multiples. Links out to the two
// new views (mutation matrix + ACM report).
//
...
export async function render(host, ctx) {
  if (!host.firstChild) host.appendChild(el('p', { class: 'dj-empty', text: 'Reading epoch contract…' }));

  const [ep, lin, traj] = await Promise.all([D.epoch(), D.lineage(), D.scoreTrajectory()]);
  if (!ep || ep.epoch_id == null) {
    gatedSwap(host, 'no-epoch', () => [el('h1', { class: 'dj-h1', text: 'Epoch' }), empty('No current epoch.')]);
    return;
  }
  ...
  gatedSwap(host, digest, () => {
    ...
    nodes.push(el('div', { class: 'dj-pagehead' }, [
      el('h1', { class: 'dj-h1', text: `Epoch ${epochId}` }),
      el('div', { class: 'dj-objective' }, [
    ...
    nodes.push(el('div', { class: 'dj-panel dj-row', style: 'margin-top:12px;' }, [
    ...
    // links to the two NEW views
    nodes.push(el('div', { class: 'dj-quicklinks' }, [
      el('a', { class: 'dj-linkbtn', href: ctx.href('mutations', {}), text: 'Mutation sites × generation →' }),
      el('a', { class: 'dj-linkbtn', href: ctx.href('report', { epochId }), text: 'Epoch publication (ACM) →' }),
    ]));

    const briefText = ep.brief || '';
    const briefDetails = el('details', { class: 'dj-brief', open: briefText.length < 1200 ? '' : null }, [
      el('summary', null, [
        el('span', { class: 'chev', text: '▸' }), 'Proposer brief',
        el('span', { class: 'dj-faint', style: 'font-weight:400;font-size:11px;', text: briefText ? `· ${briefText.split(/\n/).length} lines` : '· none' }),
      ]),
    ...
    const bumpsCard = el('div', { class: 'dj-panel' });
    bumpsCard.appendChild(svg.bumps({ width: 760, height: 170, nodes: bumpNodes, onClick: (n) => ctx.navigate('candidate', { gen: n.id }) }));
    ...
    const hmCard = el('div', { class: 'dj-panel', style: 'overflow-x:auto;' });
    if (rows.length && cols.length) {
      ...
      hmCard.appendChild(el('p', { class: 'dj-faint', style: 'font-size:11px;margin:8px 0 0;', text: 'cell = drift loss for one board entry in one generation · denser ink = more drift · click → candidate' }));
    } else {
...
function boardTrellis(board, gens, rowByGenEntry, domain, ctx) {
  if (!board.length) return el('div', { class: 'dj-panel' }, [empty('No board entries recorded.')]);
  ...
  const trellis = el('div', { class: 'dj-trellis' });
  ...
  for (const b of sorted) {
    ...
    const cell = el('figure', { class: 'dj-trellis-cell' }, [
      el('figcaption', { class: 'dj-trellis-cap' }, [
        el('span', { class: 'dj-trellis-id', text: String(eid) }),
        el('span', { class: 'dj-trellis-meta' }, [
          el('span', { class: 'dj-kind-tag dj-kind-' + (b.kind || 'unknown'), text: KIND_LABEL[b.kind] || b.kind || '—' }),
          b.expectation_kind ? el('span', { class: 'dj-faint', text: ' · ' + b.expectation_kind }) : null,
        ].filter(Boolean)),
      ]),
      svg.sparkbar({ width: 200, height: 40, bars, domain: domain || undefined }),
      svg.genDots({ width: 200, height: 14, cells }),
      el('div', { class: 'dj-trellis-foot dj-faint' }, [
        el('span', { text: svg.isNum(b.budget_s) ? `${b.budget_s}s budget` : 'no budget' }),
        el('span', { text: svg.isNum(b.weight) ? `w ${svg.fmt(b.weight, 1)}` : 'w —' }),
        Array.isArray(b.tags) && b.tags.length ? el('span', { class: 'dj-trellis-tags', text: b.tags.slice(0, 3).join(' ') }) : null,
      ].filter(Boolean)),
      b.input_preview ? el('div', { class: 'dj-trellis-preview dj-faint', title: b.input_preview, text: '“' + (b.input_preview.length > 64 ? b.input_preview.slice(0, 63) + '…' : b.input_preview) + '”' }) : null,
    ].filter(Boolean));
  ...
  card.appendChild(legend([['spine', 'one bar per candidate · drift loss (shared scale)'], ['dotact', 'pass'], ['dotpred-bad', 'fail']], '⏱ timeout · click a board → its candidate drill-down'));
  return card;
}
...
function legend(items, foot) {
  ...
  if (foot) kids.push(el('span', { class: 'dj-faint', text: foot }));
  return el('div', { class: 'dj-legend' }, kids);
}
