// variants/N/dag.js — the compact candidate-lifecycle DAG (Console).
//
...
export function lifecycleDag(spec) {
  ...
  const patchG = rectNode(nodeLayer, X.patch, midY, 0.13 * w, 44, baseline ? 'seed' : 'PATCH', patchSub, baseline ? 'ezn-baseline' : 'ezn-patch');
  // FIX #2: the PATCH node is clickable → this candidate's side-by-side diff.
  if (!baseline && o.onPatch) {
    patchG.classList.add('ezn-clickable');
    patchG.setAttribute('tabindex', '0');
    patchG.appendChild(svgEl('title', null, ['view this candidate’s patch diff →']));
    patchG.style.cursor = 'pointer';
    patchG.addEventListener('click', () => o.onPatch());
    patchG.addEventListener('keydown', (ev) => { if (ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); o.onPatch(); } });
  }
