// variants/Q/data.js — Variant Q's ("Atlas IV") read layer.
//
// Self-contained for Variant Q. Reuses the SHARED core data layer wholesale
// (core/api.js, core/state.js, core/sse.js) and adds the small set of cached,
// failure-tolerant drill-down GETs the roomy Atlas screens need — the same
// surface Variant N used, plus a tiny helper that filters /api/tournaments to
// EVERY matchup a candidate appeared in (fix #3 — champion OR challenger).
//
...
// Bust the keys that can change while a run is live (selection-change + SSE).
...
// FIX #3 — EVERY matchup a candidate appeared in. The O bug showed only one;
// here we filter the gauntlet for `champion == gen || challenger == gen` so v0
// surfaces BOTH v0→v1 AND v0→v2. Pure (no fetch): pass an already-loaded
// /api/tournaments payload.
export function matchupsForGen(bracketPayload, genId) {
  const matchups = (bracketPayload && Array.isArray(bracketPayload.matchups)) ? bracketPayload.matchups : [];
  return matchups.filter((m) => m && (m.champion === genId || m.challenger === genId));
}
