// variants/Q/views/candidate.js — CANDIDATE (one generation), the comfortable
// detail pane.
//
// Atlas IV pairs the compact lifecycle DAG with the per-board scoring dot-plot,
// then — the convergence-III fixes — adds the PROMOTE GATE on the candidate
// page (fix #1, N lacked it), a clickable PATCH node → this candidate's
// SIDE-BY-SIDE diff (fix #2, N's patch node was dead), and ALL the match-ups
// this candidate appeared in (fix #3, O showed only one).
//
// The selected entry lives in the URL so the drill-down rebuilds ONLY on a
// route change, never on a heartbeat.
//
...
import { gatePanel } from './matchups.js';
import { gatedSwap, section, subhead, empty, stat, verdictPill, normaliseDecision } from '../ui.js';

export async function render(host, ctx, params) {
  if (!host.firstChild) host.appendChild(el('p', { class: 'dq-empty', text: 'Reading candidate…' }));
  ...
  if (!ep || ep.epoch_id == null) {
    gatedSwap(host, 'no-epoch', () => [el('h1', { class: 'dq-h1', text: 'Candidate' }), empty('No current epoch.')]);
    return;
  }
  ...
  if (!node) {
    gatedSwap(host, 'no-cand', () => [el('h1', { class: 'dq-h1', text: 'Candidate' }), empty('No candidate selected.')]);
    return;
  }
  ...
  // FIX #1 — the promote gate on the candidate page.
  let gate = null;
  ...
  // FIX #3 — ALL match-ups this candidate appeared in (champion OR challenger).
  const allMatchups = D.matchupsForGen(bracket, genId);
  ...
  const digest = JSON.stringify({
    epochId, genId, parent: node.parent, decision, championId,
    ...
    matchups: allMatchups.map((m) => [m.champion, m.challenger, m.decision]),
  ...
  gatedSwap(host, digest, () => {
    ...
    nodes.push(el('div', { class: 'dq-pagehead' }, [
      el('h1', { class: 'dq-h1' }, [(node.promoted ? '♛ ' : '') + 'Candidate ' + genId]),
      el('p', { class: 'dq-lede', text: baseline ? 'The seed candidate (no parent) — it defines the loss floor for the epoch.' : `Born from ${node.parent} by a patch; faced the board; met the champion at the gate.` }),
    ]));
    ...
    nodes.push(el('div', { class: 'dq-panel dq-row' }, [
      ...
      el('div', { class: 'dq-stat' }, [verdictPill(baseline ? 'baseline' : decision)]),
    ]));

    const dagCard = el('div', { class: 'dq-panel', style: 'overflow-x:auto;' });
    dagCard.appendChild(lifecycleDag({
      ...
      width: 900, height: Math.max(300, 130 + entries.length * 34),
      onEntry: (eid) => ctx.navigate('gen', { epochId, gen: genId, entry: eid }),
      // FIX #2 — the PATCH node opens this candidate's side-by-side diff.
      onPatch: () => ctx.navigate('mutations', { epochId, gen: genId }),
    }));
    dagCard.appendChild(el('p', { class: 'dq-faint', style: 'font-size:11px;margin:10px 0 0;', text: 'parent → patch (click → this candidate’s diff) → board (one node per entry, colour = pass/fail/timeout) → Σ → gate → terminal · click a board node → its drill-down' }));
    nodes.push(section('Lifecycle · cause → effect → verdict', dagCard));

    // FIX #2 — explicit link to the side-by-side diff too (belt + suspenders).
    if (!baseline) {
      nodes.push(el('div', { class: 'dq-quicklinks' }, [
        el('a', { class: 'dq-linkbtn', href: ctx.href('mutations', { epochId, gen: genId }), text: 'What this candidate patched · side-by-side diff →' }),
        el('a', { class: 'dq-linkbtn', href: ctx.href('matchups', { epochId, gen: genId }), text: 'All match-ups for this candidate →' }),
      ]));
    }
    ...
    if (entries.length) {
      ...
      scoreCard.appendChild(svg.valueDotPlot({
        width: 560, rowHeight: 24, labelWidth: 210, items,
        reference: svg.isNum(championScalar) ? { value: championScalar, label: `champion ${championId}` } : null,
        onClick: (it) => ctx.navigate('gen', { epochId, gen: genId, entry: it.id }),
      }));
      scoreCard.appendChild(el('div', { class: 'dq-legend' }, [
        ...
        el('span', { class: 'dq-faint', text: '⏱ timeout · click an entry → its drill-down' }),
      ].filter(Boolean)));
    } else {
    ...
    if (entryParam) nodes.push(entryDrilldown(ctx, epochId, genId, entryParam, drillRow, exps, judges));

    // FIX #1 — the promote gate, stacked + non-overlapping, ON the candidate page.
    if (!baseline) {
      if (gate && Array.isArray(gate.rules)) {
        nodes.push(section('Promote gate · ' + (node.parent || 'champion') + ' → ' + genId,
          gatePanel(gate, { champion: node.parent, challenger: genId })));
      } else {
        nodes.push(section('Promote gate', el('div', { class: 'dq-panel' }, [empty('No gate decision recorded for this candidate.')])));
      }
    }

    // FIX #3 — ALL match-ups for this candidate (every round it was in).
    if (allMatchups.length) {
      ...
      tbl.appendChild(el('thead', null, [el('tr', null, [
        ...
        el('th', { class: 'dq-num', text: 'Δ scalar' }), el('th', { text: 'decision' }), el('th', { text: 'open' }),
      ])]));
      const tbody = el('tbody');
      for (const m of allMatchups) {
        const role = m.champion === genId ? 'champion' : 'challenger';
        const dec = m.decision || '—';
        const decCls = dec === 'promoted' ? 'dq-good-t' : dec === 'rejected' ? 'dq-bad-t' : '';
        tbody.appendChild(el('tr', null, [
          el('td', { text: role }),
          el('td', { class: 'dq-mono', text: m.champion || '—' }),
          el('td', { class: 'dq-mono', text: m.challenger || '—' }),
          el('td', { class: 'dq-num dq-mono', text: svg.isNum(m.delta_scalar) ? svg.fmtSigned(m.delta_scalar, 2) : '—' }),
          el('td', { class: decCls, text: dec }),
          el('td', null, [el('a', { class: 'dq-linkbtn dq-board-run', href: ctx.href('matchups', { epochId, gen: m.challenger === genId ? genId : m.champion }), text: 'duel →' })]),
        ]));
      }
      tbl.appendChild(tbody);
      card.appendChild(tbl);
      card.appendChild(el('p', { class: 'dq-faint', style: 'font-size:11px;margin:10px 0 0;', text: `${allMatchups.length} match-up${allMatchups.length === 1 ? '' : 's'} · every round this candidate appeared in (champion or challenger)` }));
      nodes.push(section('All match-ups for this candidate', card));
    }
    return nodes;
  });
}
...
function entryDrilldown(ctx, epochId, genId, entryId, row, exps, judges) {
  ...
  const card = el('div', { class: 'dq-panel dq-drill' });
  card.appendChild(el('div', { class: 'dq-row' }, [
  ...
  if (outcomes.length) {
    const grid = el('div', { class: 'dq-expect-grid', style: 'margin-top:14px;' });
    for (const o of outcomes) {
      const cls = o.passed === true ? 'dq-good' : o.passed === false ? 'dq-bad' : 'dq-flat';
      grid.appendChild(el('div', { class: 'dq-expect-row' }, [
        el('span', { class: 'dq-expect-dot ' + cls, title: o.passed === true ? 'passed' : o.passed === false ? 'failed' : 'no verdict' }),
        el('span', { class: 'dq-expect-kind', text: o.kind || 'expectation' }),
        o.judge_name ? el('span', { class: 'dq-faint', text: ' · ' + o.judge_name }) : null,
        el('span', { class: 'dq-expect-detail dq-faint', text: o.detail ? ' — ' + o.detail : '' }),
      ].filter(Boolean)));
    }
    card.appendChild(grid);
  } else {
    card.appendChild(el('div', { style: 'margin-top:14px;' }, [empty('No expectation recorded for this entry (no predicate / rubric).')]));
  }
  ...
  if (jitems.length) {
    card.appendChild(el('p', { class: 'dq-faint', style: 'margin:16px 0 4px;font-size:11px;', text: 'per-judge weighted process-drift loss · higher = more drift' }));
    card.appendChild(svg.valueBars({ width: 420, rowHeight: 22, labelWidth: 180, items: jitems }));
  }

  card.appendChild(el('div', { style: 'margin-top:16px;' }, [
    el('a', { class: 'dq-linkbtn', href: ctx.href('run', { epochId, gen: genId, entry: entryId }), text: 'Open the full run transcript →' }),
    runId ? el('span', { class: 'dq-faint dq-mono', style: 'margin-left:8px;', text: runId.slice(0, 10) + '…' }) : null,
  ].filter(Boolean)));
