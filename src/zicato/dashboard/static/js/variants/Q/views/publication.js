// variants/Q/views/publication.js — ACM-style epoch publication.
//
// Reuses K's publication renderer approach (judged best): parse the section
// markers; typeset eyebrow / title / meta / abstract / body; splice live Tufte
// figures at <!-- FIGURE:NAME --> markers. GFM tables render. The aggregate
// generation-scores TABLE and its summary BAR CHART are COMBINED into ONE
// cohesive visual; per-matchup detail is appended from the matchup grid.
// Epoch-scoped (reachable from the tree's Publication node). Cold deep-link safe.
//
...
export async function render(host, ctx, params) {
  if (!host.firstChild) host.appendChild(el('p', { class: 'dq-empty', text: 'Reading epoch publication…' }));
  ...
  if (!epochId) {
    gatedSwap(host, 'no-epoch', () => [el('h1', { class: 'dq-h1', text: 'Epoch publication' }), empty('No epoch selected.')]);
    return;
  }
  ...
  gatedSwap(host, digest, () => {
    ...
    const article = el('article', { class: 'dq-paper' });

    const masthead = el('header', { class: 'dq-paper-masthead' });
    if (paper.eyebrow) masthead.appendChild(el('div', { class: 'dq-paper-eyebrow', text: paper.eyebrow }));
    masthead.appendChild(el('h1', { class: 'dq-paper-title', text: paper.title || `Epoch ${epochId}` }));
    masthead.appendChild(el('div', { class: 'dq-paper-rule' }));
    if (paper.meta.length) {
      masthead.appendChild(el('div', { class: 'dq-paper-metagrid' }, paper.meta.map((m) => el('div', { class: 'dq-paper-meta-cell' }, [
        m.label ? el('span', { class: 'dq-paper-meta-label', text: m.label }) : null,
        el('span', { class: 'dq-paper-meta-value', text: m.value }),
      ].filter(Boolean)))));
    }
    article.appendChild(masthead);

    if (!md.trim()) {
      article.appendChild(el('div', { class: 'dq-paper-statebox' }, [
        el('p', { class: 'dq-paper-statebox-h', text: 'The narrative report has not been written yet for ' + epochId + '.' }),
        el('p', { class: 'dq-faint' }, [
          'Run ', el('code', { class: 'dq-paper-code', text: 'zicato epoch analyze --epoch ' + epochId }),
    ...
    if (paper.abstract) {
      article.appendChild(el('section', { class: 'dq-paper-abstract' }, [
        el('div', { class: 'dq-paper-abstract-label', text: 'Abstract' }),
    ...
    article.appendChild(el('div', { class: 'dq-paper-body' }, [body]));
...
function figureFor(name, figures) {
  ...
  if ((key.includes('lineage') || key.includes('bump')) && gens.length) {
    const fig = el('figure', { class: 'dq-paper-fig' });
    ...
    fig.appendChild(svg.bumps({ width: 640, height: 170, nodes: bumpNodes, onClick: (n) => ctx.navigate('gen', { epochId, gen: n.id }) }));
    fig.appendChild(el('figcaption', { class: 'dq-paper-figcap', text: 'Figure · lineage bumps — champion spine vs rejected challengers (click a node → candidate).' }));
    return fig;
  }
  ...
  if ((key.includes('matchup') || key.includes('slope') || key.includes('duel')) && g0 && m0) {
    ...
    fig.appendChild(svg.pairedSlopegraph({ width: 520, height: Math.max(200, 50 + series.length * 28),
      left: { title: `champion ${m0.champion}` }, right: { title: `challenger ${m0.challenger}` }, labelGap: 150, goodDirection: 'down', series,
      onClick: (s) => ctx.navigate('run', { epochId, gen: m0.challenger, entry: s.id }) }));
    fig.appendChild(el('figcaption', { class: 'dq-paper-figcap', text: `Figure · paired per-board duel ${m0.champion} → ${m0.challenger}.` }));
    return fig;
  }
  if ((key.includes('drift') || key.includes('heatmap')) && g0 && m0) {
    ...
    fig.appendChild(el('figcaption', { class: 'dq-paper-figcap', text: 'Figure · per-board drift loss heatmap (champion vs challenger; theme-aware ink).' }));
    return fig;
  }
  return null;
}
...
function aggregateScoresFigure(gens, scalarByGen) {
  const fig = el('figure', { class: 'dq-paper-fig dq-scores-fig' });
  const items = gens.map((g) => ({ id: g.id, label: g.id, promoted: g.promoted, value: scalarByGen.get(g.id) }))
    .filter((it) => svg.isNum(it.value));
  if (!items.length) {
    fig.appendChild(el('figcaption', { class: 'dq-paper-figcap dq-faint', text: 'Figure · aggregate generation scores (no trajectory data yet).' }));
    return fig;
  }
  const combined = el('div', { class: 'dq-scores-combined' });
  combined.appendChild(svg.valueBars({
    width: 300, rowHeight: 26, labelWidth: 60,
    items: items.map((it) => ({ label: it.label + (it.promoted ? ' ♛' : ''), value: it.value })),
  }));
  const tbl = el('table', { class: 'dq-md-table dq-scores-table' });
  tbl.appendChild(el('thead', null, [el('tr', null, [el('th', { text: 'generation' }), el('th', { class: 'dq-num', text: 'scalar (loss)' }), el('th', { text: 'outcome' })])]));
  const tbody = el('tbody');
  for (const it of items) {
    tbody.appendChild(el('tr', null, [
      el('td', { class: 'dq-mono', text: it.label }),
      el('td', { class: 'dq-num dq-mono', text: svg.fmt(it.value, 2) }),
  ...
  fig.appendChild(el('figcaption', { class: 'dq-paper-figcap', text: 'Figure · aggregate generation scores — the summary bar chart and its table are one visual (lower scalar is better).' }));
  return fig;
}

function appendCanonicalFigures(article, figures) {
  const plates = el('section', { class: 'dq-paper-plates' }, [
    el('h2', { class: 'dq-paper-h2', text: 'Figures' }),
    el('p', { class: 'dq-faint', text: 'Live, interactive — click any figure to drill into the dashboard.' }),
  ]);
...
function appendMatchupDetail(article, figures) {
  ...
  const sec = el('section', { class: 'dq-paper-matchups' });
  sec.appendChild(el('h2', { class: 'dq-paper-h2', text: 'Per-match-up detail' }));
  let any = false;
  matchups.forEach((m, i) => {
    ...
    const tbl = el('table', { class: 'dq-md-table' });
    tbl.appendChild(el('thead', null, [el('tr', null, [
      el('th', { text: 'board entry' }), el('th', { class: 'dq-num', text: m.champion }),
      el('th', { class: 'dq-num', text: m.challenger }), el('th', { class: 'dq-num', text: 'Δ' }), el('th', { text: 'verdict' }),
    ])]));
    const tbody = el('tbody');
    for (const r of rows) {
      ...
      const vCls = r.verdict === 'improved' ? 'dq-good-t' : r.verdict === 'regressed' ? 'dq-bad-t' : '';
      const tr = el('tr', null, [
        el('td', { class: 'dq-mono', text: r.entry_id }),
        el('td', { class: 'dq-num dq-mono', text: svg.isNum(r.parent_drift_loss) ? svg.fmt(r.parent_drift_loss, 1) : '—' }),
        el('td', { class: 'dq-num dq-mono', text: svg.isNum(r.child_drift_loss) ? svg.fmt(r.child_drift_loss, 1) : '—' }),
        el('td', { class: 'dq-num dq-mono ' + vCls, text: svg.isNum(d) ? svg.fmtSigned(d, 1) : '—' }),
        el('td', { class: vCls, text: r.verdict || '—' }),
      ]);
      tr.style.cursor = 'pointer';
      tr.addEventListener('click', () => ctx.navigate('run', { epochId, gen: m.challenger, entry: r.entry_id }));
