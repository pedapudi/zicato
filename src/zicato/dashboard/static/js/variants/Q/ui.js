// variants/Q/ui.js — shared chrome helpers for the Atlas IV variant.
//
// Self-contained for Variant Q ("Atlas IV") — the roomy, comfortable take: N's
// content + diagrams with M's generous spacing/proportion. Small, pure builders
// every view composes: the digest-gated content swap (the no-flash guarantee),
// section headers, verdict pills, a GFM-capable tiny-markdown renderer (TABLES
// render), honest empty / loading states, and the colour + typeface theme
// tables. No data fetching, no state mutation.
//
// Chrome classes are `dq-*`; the SVG data marks (from svg.js / dag.js) keep
// their `dn-*` / `ezn-*` names and are styled — scoped under Variant Q's root —
// by css/variants/Q/atlas4.css. Q defaults Solarized-Dark + Sans.
...
export function gatedSwap(host, digest, build) {
  ...
  if (host.getAttribute('data-q-digest') === next && host.firstChild) return false;
  clearChildren(host);
  const built = build();
  const nodes = Array.isArray(built) ? built : [built];
  for (const n of nodes) { if (n) host.appendChild(n); }
  host.setAttribute('data-q-digest', next);
  return true;
}

// ---- colour themes (solarized-dark is Q's default) ------------------
...
const COLOR_KEY = 'zicato.Q.theme';
...
// ---- typeface themes (Sans is Q's default) --------------------------
//
// Each maps to a `[data-q-type]` value the stylesheet keys on (swapping the
// --q-font-* custom properties). All are Open-Sans-based for the body; the
// distinction is the heading / data voice. Q defaults Sans (comfortable).
...
export const DEFAULT_TYPE = 'sans';
const TYPE_KEY = 'zicato.Q.typeface';
...
export function section(titleText, ...children) {
  return el('section', { class: 'dq-section' }, [
    el('h2', { class: 'dq-h2', text: titleText }),
    ...children.filter(Boolean),
  ]);
}

export function subhead(text) { return el('div', { class: 'dq-subhead', text }); }

export function empty(text) {
  return el('p', { class: 'dq-empty', text: text || 'No data yet.' });
}

export function loading(text) {
  return el('p', { class: 'dq-empty', text: text || 'Loading…' });
}
...
export function verdictPill(decision) {
  ...
  return el('span', { class: `dq-pill dq-${d}`, text: label });
}

export function stat(value, key) {
  return el('div', { class: 'dq-stat' }, [
...
export function linkButton(text, hrefStr, attrs) {
  return el('a', Object.assign({ class: 'dq-linkbtn', href: hrefStr }, attrs || {}), [text]);
}

// ---- tiny markdown → DOM (GFM tables render) ------------------------
export function renderMarkdown(md, opts) {
  const o = opts || {};
  const root = el('div', { class: 'dq-md-body' });
  ...
  flushPara(); flushList();
  if (!root.firstChild) root.appendChild(el('p', { class: 'dq-faint', text: 'Nothing to render.' }));
  return root;
}
...
function table(header, rows) {
  const t = el('table', { class: 'dq-md-table' });
...
function inline(s) {
  ...
  while ((m = re.exec(str)) !== null) {
    ...
    if (tok.startsWith('`')) out.push(el('code', { text: tok.slice(1, -1) }));
      ...
      const lm = /^\[([^\]]+)\]\(([^)]+)\)$/.exec(tok);
      if (lm) out.push(el('a', { class: 'dq-md-link', href: lm[2], text: lm[1] }));
      else out.push(tok);
    }
