// variants/I/svg.js — dependency-free SVG data-viz primitives (Ledger).
//
// Self-contained for Variant I ("Ledger"). Mark CSS classes are `d-*` and
// are styled — scoped under the variant root — by css/variants/I/ledger.css.
//
// Ledger is the editorial, light-first publication skin built on Variant
// E's flow, so it shares E's "Tufte data-visualization, done beautifully"
// drawing language: high data-ink, minimal chrome, small multiples,
// sparklines, range-frames, and a *non-colliding* slopegraph / bumps
// chart — re-skinned for an airy, publication voice. This module is the
// entire drawing toolkit — pure functions that return detached SVG nodes
// built with the core `svgEl` helper. No external charting library, no
// canvas; every mark is an addressable SVG node so hover and click are
// first-class.
//
// Conventions:
...
//     mark, never a throw.
//   * Colours come from CSS custom properties read at draw time, so a
//     mark re-skins for ALL THREE themes (solarized-light default,
//     solarized-dark, monokai) with sufficient contrast in each. The
//     heatmap/dot ramp interpolates between LIVE theme tokens
//     (`--i-ramp-lo` … `--i-ramp-hi`) rather than hard-coded hex — the
//     round-3 "per-board readable in all three themes" requirement.
...
// Quiet sequential / diverging ramp. Reads LIVE theme tokens off the
// variant root so the same loss-cell reads with sufficient contrast in
// all three Ledger themes (solarized-light default, solarized-dark,
// monokai). Tokens (with light-theme fallbacks baked in here so a unit
// test without a stylesheet still draws a sane ramp):
//   --i-ramp-lo  low loss / good end   (light: pale paper)
//   --i-ramp-hi  high loss / ink end   (light: warm ink)
//   --i-ramp-good / --i-ramp-bad  the diverging endpoints.
...
function rampColor(t, diverging) {
  ...
  if (diverging) {
    const good = themeVar('--i-ramp-good', '#1f7a6b');
    const mid = themeVar('--i-ramp-lo', '#e9e3d6');
    const bad = themeVar('--i-ramp-bad', '#b14a4a');
    if (x < 0.5) return lerpHex(good, mid, x * 2);
    return lerpHex(mid, bad, (x - 0.5) * 2);
  }
  // HIGH loss = more ink (the warm/darker token end).
  return lerpHex(themeVar('--i-ramp-lo', '#f2ede1'), themeVar('--i-ramp-hi', '#9c5a3c'), x);
}
