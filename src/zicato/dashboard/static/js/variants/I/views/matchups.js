// variants/I/views/matchups.js — MATCH-UPS across tournament styles.
//
// zicato RUNS a king-of-the-hill gauntlet (one reigning champion per epoch,
// one challenger per round, paired / common-random-number duels). Ledger's
// match-ups screen, in its editorial voice:
//   1. the gauntlet LADDER — a non-colliding bumps chart (a figure);
//   2. per-round PAIRED SLOPEGRAPHS — champion loss → challenger loss for
//      each board entry, de-collided + jittered so coincident values stay
//      legible; click a line → its run;
//   3. ALTERNATIVE styles (illustrative overlays on the SAME candidate set):
//      a single-elim bracket, a round-robin matrix, race lanes — clearly
...
export async function render(host, ctx) {
  ...
  gatedSwap(host, digest, () => {
    ...
    out.push(section('The gauntlet ladder · real data', el('div', { class: 'd-panel' }, [
    ...
    if (anyGrid) slopeCard.appendChild(el('p', { class: 'i-figcap', text: 'Each line is one board entry, champion loss on the left, challenger loss on the right; lines are de-collided + jittered so coincident values stay legible. Click a line to open its run.' }));
