// variants/I/views/epoch.js — EPOCH: the data substrate of one epoch.
//
// Ledger's epoch screen reads like the opening pages of a paper: an
// eyebrow + serif title, the objective as a lede, the proposer brief, then
// the three figures (each with an editorial caption) carried over from E's
// data-viz — the non-colliding lineage BUMPS, the board × generation drift
// HEATMAP, and the board TRELLIS. A prominent rail links to the epoch's
// ACM-style publication.
//
...
export async function render(host, ctx) {
  ...
  gatedSwap(host, digest, () => {
    ...
    // publication rail
    nodes.push(el('div', { class: 'i-paper-rail' }, [
      el('span', { class: 'i-paper-rail-lab', text: 'Publication' }),
      el('span', { class: 'd-faint', text: 'The full ACM-style write-up of this epoch, with live figures, is its own tab.' }),
      el('a', { class: 'i-btn', href: ctx.href('paper', { epochId }), text: 'Read the publication →' }),
    ]));
    ...
    nodes.push(section('Lineage', el('div', { class: 'd-panel' }, [
    ...
    if (rows.length && cols.length) {
      ...
      nodes.push(section('Board entries × generations · drift loss', el('div', { class: 'd-panel' }, [
        figure(hmInner, 'Each cell is the drift loss for one board entry in one generation; darker is more drift. Click a cell to open its candidate.', { label: 'Figure 3.' }),
      ])));
    } else {
...
function boardTrellis(board, gens, rowByGenEntry, domain, ctx) {
  ...
  card.appendChild(el('p', { class: 'i-figcap', text: 'One micro bar per candidate, drift loss on a shared scale; the glyph row reads pass / fail / timeout. Click a board to open its candidate.' }));
  return card;
}
