// variants/G/components/palette.js — ⌘K command palette (Bridge).
//
// The jump-anywhere palette A's navigation was praised for. Built from
// the live state (epochs list + current epoch's experiments) so it
// always reflects the real workspace. ⌘K / Ctrl-K to open, ↑↓ to move,
// ↵ to go, Esc to close.
...
function buildItems() {
  ...
  for (const e of epochs) {
    ...
    out.push({ kind: 'match-ups', label: 'match-ups · ' + e.epoch_id, href: href('tournament', { epochId: e.epoch_id }) });
  }
  const def = state.epochDef;
  if (def && Array.isArray(def.experiments)) {
    for (const x of def.experiments) {
      if (!x || !x.generation_id) continue;
      out.push({ kind: 'candidate', label: (def.epoch_id || '') + ' · ' + x.generation_id, href: href('experiment', { epochId: def.epoch_id, genId: x.generation_id }) });
    }
  }
  return out;
}
...
function render() {
  ...
  items.forEach((it, i) => {
    const node = el('div', {
      class: 'g-palette-item' + (i === active ? ' is-active' : ''),
      role: 'option',
    }, [
      el('span', { class: 'g-palette-item-kind' }, [it.kind]),
      el('span', { class: 'g-palette-item-label' }, [it.label]),
    ]);
...
export function mountPalette(rootHost) {
  overlay = el('div', { class: 'g-palette-overlay', hidden: 'true', role: 'dialog', 'aria-modal': 'true' });
  input = el('input', { class: 'g-palette-input', type: 'search', placeholder: 'Jump to an epoch, candidate, or page…', autocomplete: 'off' });
  results = el('div', { class: 'g-palette-results', role: 'listbox' });
  const pal = el('div', { class: 'g-palette' }, [
    input, results,
    el('div', { class: 'g-palette-foot' }, ['↑↓ navigate   ↵ open   esc close']),
  ]);
