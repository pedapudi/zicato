// variants/G/components/markdown.js — tiny, safe markdown -> DOM.
//
// The proposer brief is authored markdown and can be LONG. It deserves
// a real, readable home — not a truncated line. Renders a useful subset
// (headings, paragraphs, lists, fenced code, inline code, bold, italic)
// into DOM nodes WITHOUT innerHTML, so it cannot inject markup.
...
export function renderMarkdown(md) {
  const root = el('div', { class: 'g-prose' });
  ...
  while (i < lines.length) {
    ...
    if (/^```/.test(line)) {
      ...
      i += 1;
      root.appendChild(el('pre', null, [el('code', { class: 'g-mono' }, [buf.join('\n')])]));
      continue;
    }
  ...
  flushPara(); flushList();
  if (root.childNodes.length === 0) return el('div', { class: 'g-empty' }, ['(empty)']);
  return root;
}
