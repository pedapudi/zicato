// variants/F/views/experiment.js — Screen 3: THE causal flow.
//
...
export function renderExperiment(ctx) {
  ...
  // Digest gate: gen id, hypothesis core idea, decision, the loaded drift
  // movements, the outcome deltas. No timestamps — a heartbeat is a no-op.
  const drift = _driftCache.get(genId);
  const mv = (drift && Array.isArray(drift.movements)) ? drift.movements : null;
  const out = (exp && exp.outcome) || null;
  const digest = JSON.stringify({
    epochId, genId,
    core: exp && exp.hypothesis ? exp.hypothesis.core_idea || '' : '',
    dec: exp ? decisionOf(exp) : null,
    mv: mv ? mv.map((m) => [m.kind, m.delta, m.direction]) : null,
    deltas: out ? [out.scalar_score_delta, out.drift_loss_delta, out.pass_rate_delta] : null,
    reason: out ? (out.rejection_reason || out.decision_reason || '') : '',
  });
