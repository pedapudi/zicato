// variants/F/views/lifecycle.js — Screen: the CANDIDATE LIFECYCLE (themes 1+2).
//
...
import { valueDotPlot, sparkbar, extent } from '../lib/svg.js';
import { pullQuote } from '../lib/prose.js';
...
export function renderLifecycle(ctx) {
  ...
  // Digest gate: the selected candidate, its lineage record, the loaded
  // board rows, and the gate decision — never raw timestamps. A heartbeat
  // tick with unchanged structure is a no-op.
  const nodePre = lineageNode(state, genId);
  const rowsPre = _entryCache.get(key(epochId, genId)) || null;
  const expPre = experimentById(state, genId);
  const gatePre = nodePre && nodePre.parent
    ? _gateCache.get(gkey(epochId, nodePre.parent, genId)) : null;
  const digest = JSON.stringify({
    epochId, genId,
    node: nodePre ? [nodePre.parent, nodePre.promoted] : null,
    rows: rowsPre ? rowsPre.map((r) => [r.entryId, r.driftLoss, r.passFail, r.budgetExceeded]) : null,
    dec: expPre ? decisionOf(expPre) : null,
    gate: gatePre ? gatePre.decision : null,
    cands: candidatesOf(state, epochId).map((c) => [c.id, c.parent, c.promoted]),
  });
  ...
  if (node.parent) ensureEntries(epochId, node.parent, repaint); // champion reference level
  ...
  // Editorial pull-quote: the hypothesis (the bet the proposer made).
  const hyp = (exp && exp.hypothesis) || {};
  if (hyp.core_idea) {
    stage.appendChild(pullQuote(String(hyp.core_idea), { attribution: 'the hypothesis for ' + genId }));
  }

  // The hero: C's lifecycle spine (patch → board → aggregate → gate).
  stage.appendChild(buildLifecycleSpine(state, epochId, genId, node, exp, baseline));

  // Editorial pull-quote: the gate's verdict reason, in the gate's voice.
  const gate = node.parent ? _gateCache.get(gkey(epochId, node.parent, genId)) : null;
  const reason = (gate && gate.reason)
    || (exp && exp.outcome && (exp.outcome.rejection_reason || exp.outcome.decision_reason))
    || null;
  if (!baseline && reason) {
    const dec = (gate && gate.decision) || decisionOf(exp) || 'rejected';
    stage.appendChild(el('div', { class: 'czF-gate-quote ' + verdictClass(dec) }, [
      pullQuote(String(reason), { attribution: 'the gate · ' + verdictLabel(dec).toLowerCase() }),
    ]));
  }

  // D's evidence, embedded: the per-board loss profile as supporting data.
  stage.appendChild(buildBoardEvidence(state, epochId, genId, node));

  stage.appendChild(buildLineageSection(state, epochId, genId, onNavigate));
}

// -- D evidence: the per-board loss profile (embedded supporting data) ---
function buildBoardEvidence(state, epochId, genId, node) {
  const wrap = el('div', { 'data-cz': 'board-evidence' }, [
    el('h2', { class: 'cz-section-title' }, ['The evidence']),
    el('p', { class: 'cz-section-sub' }, [
      'How this candidate scored on every board entry it faced — the numbers '
      + 'behind the flow above. Lower loss is better; ✓ passed, ✗ failed, ⏱ timed out.',
    ]),
  ]);
  const rows = _entryCache.get(key(epochId, genId));
  if (rows == null) {
    wrap.appendChild(el('div', { class: 'cz-empty cz-empty-inline' }, ['Loading per-board scores…']));
    return wrap;
  }
  if (rows.length === 0) {
    wrap.appendChild(el('div', { class: 'cz-empty cz-empty-inline' }, ['No board entries scored for this candidate yet.']));
    return wrap;
  }

  // A sorted value dot-plot (worst-first) — Tufte's high-data-ink read.
  const items = rows
    .slice()
    .sort((a, b) => (b.driftLoss || 0) - (a.driftLoss || 0))
    .map((r) => ({
      id: r.entryId, label: r.entryId, value: r.driftLoss,
      pass: r.passFail, timeout: r.budgetExceeded,
    }));
  const champLoss = node.parent ? championLoss(state, epochId, node.parent) : null;
  const panel = el('div', { class: 'czF-evidence-panel' });
  panel.appendChild(valueDotPlot({
    width: 560, rowHeight: 22, labelWidth: 200, items,
    reference: champLoss != null ? { value: champLoss, label: 'champion ' + node.parent } : null,
    onClick: (it) => {
      const r = rows.find((x) => x.entryId === it.id);
      if (r && r.runId) window.location.hash = href('run', { runId: r.runId });
    },
  }));

  // A word-sized sparkbar summarising the whole profile + verdict glyph.
  const dom = extent(rows.map((r) => r.driftLoss));
  panel.appendChild(el('div', { class: 'czF-evidence-spark' }, [
    el('span', { class: 'cz-board-kind' }, ['loss profile']),
    sparkbar({
      width: 200, height: 34, domain: dom,
      bars: rows.map((r) => ({ label: r.entryId, value: r.driftLoss, fail: r.passFail === 0, timeout: r.budgetExceeded })),
      verdict: node.promoted === true ? 'promoted' : (node.promoted === false ? 'rejected' : null),
    }),
  ]));
...
// The champion's aggregate loss (reference line) — its summed board loss.
function championLoss(state, epochId, champId) {
  const rows = _entryCache.get(key(epochId, champId));
  if (!rows || !rows.length) return null;
  // Per-entry mean as a comparable reference level on the same axis.
  const vals = rows.map((r) => r.driftLoss).filter((v) => typeof v === 'number');
