// variants/W/ui.js — shared chrome helpers for the Arena variant.
//
// Self-contained for Variant W (ported from P/N's helpers). Small, pure builders
...
//
// W keeps the dense Console mark classes (`dn-*`) so the ported svg/dag/views
// read unchanged, but the SHELL chrome + the broadcast standings carry their own
// `dw-*` classes scoped under the Variant-W root. Default colour: monokai (a
// broadcast-dark canvas; solarized-dark is the calmer alternate). Default
// typeface: Display (Archivo Narrow headings + big numbers — the billboard
// voice).
...
export function gatedSwap(host, digest, build) {
  ...
  if (host.getAttribute('data-w-digest') === next && host.firstChild) return false;
  clearChildren(host);
  const built = build();
  const nodes = Array.isArray(built) ? built : [built];
  for (const n of nodes) { if (n) host.appendChild(n); }
  host.setAttribute('data-w-digest', next);
  return true;
}

// ---- colour themes (monokai is W's default; sol-dark the alternate) -
...
const COLOR_KEY = 'zicato.W.theme';
...
// ---- typeface themes (Display is W's default — the billboard voice) -
...
const TYPE_KEY = 'zicato.W.typeface';
