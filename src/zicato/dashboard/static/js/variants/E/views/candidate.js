...
export async function render(host, ctx, params) {
  ...
  gatedSwap(host, digest, () => {
    ...
    const dagCard = el('div', { class: 'd-panel', style: 'overflow-x:auto;' });
...
function entryDrilldown(ctx, genId, entryId, row, exps, judges) {
  ...
  // a link into the full run transcript (the run view)
  card.appendChild(el('div', { style: 'margin-top:14px;' }, [
    el('a', { class: 'd-link', href: ctx.href('run', { gen: genId, entry: entryId }), text: 'Open the full run transcript →' }),
    runId ? el('span', { class: 'd-faint d-mono', style: 'margin-left:8px;', text: runId.slice(0, 10) + '…' }) : null,
  ].filter(Boolean)));
