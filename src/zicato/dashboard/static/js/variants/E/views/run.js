// variants/E/views/run.js — RUN DETAIL: the transcript.
//
// The deepest screen: one run's reconstructed conversation (turns, tool
// calls, drift annotations). Deep-link hydration is first-class — on a COLD
// deep-link (e.g. #/E/run/v1/waffles_single opened directly) this view
// resolves the run_id from /api/generation/.../per-entry, then FETCHES
// /api/conversation/{run_id} itself: loading → content, NEVER an empty
// panel. The transcript turns render in normal block flow inside a single
// constrained, scrollable container (height + overflow-y:auto) — no absolute
// positioning that could overlap lines.
//
// Data: /api/generation/{e}/{g}/per-entry (the run_id),
// /api/conversation/{run_id} (the transcript). Optionally the live event
// tail from AppState for an in-flight run.
...
export async function render(host, ctx, params) {
  ...
  if (!genId || !entryId) {
    gatedSwap(host, 'no-run', () => [
      el('h1', { class: 'd-h1', text: 'Run' }),
      empty('No run selected — open a run from a candidate’s per-board scoring or a match-up duel.'),
    ]);
    return;
  }

  // Resolve the run_id from the per-entry index, then fetch the transcript.
  // On a cold deep-link this is the chain that hydrates the screen from the
  // URL alone, so the transcript loads without any prior navigation.
  ...
  gatedSwap(host, digest, () => {
    ...
    nodes.push(el('div', { class: 'e-pagehead' }, [
      el('h1', { class: 'd-h1', text: `Run · ${entryId}` }),
      el('p', { class: 'd-lede' }, [
        `The reconstructed conversation for ${genId} on this board entry`,
        runId ? el('span', { class: 'd-faint d-mono', text: '  ' + runId.slice(0, 12) + '…' }) : null,
      ].filter(Boolean)),
    ]));
...
function transcriptPanel(conv, runId, turns, anns) {
  ...
  // Annotations indexed by the turn they anchor to.
  ...
  // One constrained, scrollable container; turns flow as normal blocks.
