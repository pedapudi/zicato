// variants/E/ui.js — shared chrome helpers for the Atlas variant.
//
// Self-contained for Variant E. Small, pure builders that every view
// composes: section headers, the
...
//
// Re-render discipline: a view computes a stable digest of ONLY its
// structural/content data (timestamps / heartbeat fields EXCLUDED), then
// calls gatedSwap(host, digest, build). If the digest equals the one this
// host last painted AND the host still has children, NOTHING is written —
// so a steady heartbeat tick that re-dispatches the active view is a true
// no-op and the screen cannot flash. Only when the digest changes does the
// host get cleared and rebuilt once from `build()` (an array of nodes).
//
// The digest is stored on the host's `data-e-digest` attribute, so the
// gate survives across re-renders without any module-level bookkeeping.
export function gatedSwap(host, digest, build) {
  if (!host) return false;
  const next = String(digest);
  if (host.getAttribute('data-e-digest') === next && host.firstChild) return false;
  clearChildren(host);
  const built = build();
  const nodes = Array.isArray(built) ? built : [built];
  for (const n of nodes) { if (n) host.appendChild(n); }
  host.setAttribute('data-e-digest', next);
  return true;
}
