// variants/S/tree.js — the HEADLINE: a persistent data-model TREE sidebar.
//
...
const OPEN_KEY = 'zicato.S.tree.open';
...
export function paintTree(host, m, route, nav) {
  ...
  if (host.getAttribute('data-s-tree-digest') === dg && host.firstChild) return false;
  clearChildren(host);
  host.setAttribute('data-s-tree-digest', dg);

  const sel = selectionKey(route);
  const tree = el('div', { class: 'vs-tree', role: 'tree', 'aria-label': 'Data model' });
...
function branchRow(o) {
  const row = el('div', { class: 'vs-tnode vs-depth-' + o.depth + (o.selected ? ' vs-tsel' : '') });
  const tw = el('button', { class: 'vs-ttwist', type: 'button', 'aria-expanded': String(o.open), title: o.open ? 'collapse' : 'expand', text: o.open ? '▾' : '▸' });
  tw.addEventListener('click', (ev) => { ev.preventDefault(); ev.stopPropagation(); toggle(o.key, o.host, o.m, o.route, o.nav); });
  const link = el('a', { class: 'vs-tlabel', href: href(o.view, o.params) });
  link.appendChild(el('span', { class: 'vs-tglyph', text: o.glyph }));
  link.appendChild(el('span', { class: 'vs-ttext', text: o.label }));
  if (o.sub) link.appendChild(el('span', { class: 'vs-tsub', text: o.sub }));
...
function groupRow(o) {
  const row = el('div', { class: 'vs-tnode vs-tgroup vs-depth-' + o.depth });
  const tw = el('button', { class: 'vs-ttwist', type: 'button', 'aria-expanded': String(o.open), title: o.open ? 'collapse' : 'expand', text: o.open ? '▾' : '▸' });
  tw.addEventListener('click', (ev) => { ev.preventDefault(); toggle(o.key, o.host, o.m, o.route, o.nav); });
  const lbl = el('button', { class: 'vs-tlabel vs-tgrouplbl', type: 'button' }, [
    el('span', { class: 'vs-ttext', text: o.label }),
    el('span', { class: 'vs-tcount', text: String(o.count) }),
  ]);
...
function leaf(o) {
  const row = el('div', { class: 'vs-tnode vs-tleaf vs-depth-' + o.depth + (o.selected ? ' vs-tsel' : '') + (o.dim ? ' vs-tdim' : '') });
  row.appendChild(el('span', { class: 'vs-ttwist vs-ttwist-empty', 'aria-hidden': 'true' }));
  const link = el('a', { class: 'vs-tlabel', href: href(o.view, o.params) });
  link.appendChild(el('span', { class: 'vs-tglyph', text: o.glyph }));
  link.appendChild(el('span', { class: 'vs-ttext', text: o.label }));
  if (o.sub) link.appendChild(el('span', { class: 'vs-tsub', text: o.sub }));
...
function toggle(key, host, m, route, nav) {
  ...
  host.removeAttribute('data-s-tree-digest');
