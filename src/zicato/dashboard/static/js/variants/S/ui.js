// variants/S/ui.js — shared chrome helpers for Variant S ("Lens").
//
// Self-contained for Variant S. Small, pure builders the comparison-first
// detail panes compose: the digest-gated content swap (the no-flash guarantee,
// usable independently per comparison side), section headers, verdict pills, a
// GFM-capable tiny-markdown renderer (TABLES render), honest empty / loading
// states, and the colour + typeface theme tables.
//
// S defaults to **solarized-light** colour + **Editorial** typeface. The marks
// (svg.js / dag.js, carried from the praised Console base) emit `dn-*` / `ezn-*`
// classes that read the shared `--v2-*` tokens; S's chrome uses `vs-*`. Both
// pickers persist under S-scoped localStorage keys. No data fetching here.
...
// true no-op. Each comparison side passes its OWN host, so the two sides gate
// independently. The data attribute is per-host so two panes never collide.
export function gatedSwap(host, digest, build) {
  if (!host) return false;
  const next = String(digest);
  if (host.getAttribute('data-s-digest') === next && host.firstChild) return false;
  clearChildren(host);
  const built = build();
  const nodes = Array.isArray(built) ? built : [built];
  for (const n of nodes) { if (n) host.appendChild(n); }
  host.setAttribute('data-s-digest', next);
  return true;
}

// ---- colour themes (solarized-light is S's default) -----------------
...
export const DEFAULT_COLOR = 'solarized-light';
const COLOR_KEY = 'zicato.S.theme';
...
// ---- typeface themes (Editorial is S's default) ---------------------
//
// Each maps to a `[data-s-type]` value the stylesheet keys on (swapping the
// --s-font-* / --v2-sans / --v2-mono custom properties). All Open-Sans-based
// for the body; the distinction is the heading / data voice:
//   Sans      — Open Sans throughout (tabular figures for data).
//   Editorial — Open Sans body + Source Serif 4 headings & publication (S).
...
export const DEFAULT_TYPE = 'editorial';
const TYPE_KEY = 'zicato.S.typeface';
...
// A themed link/button — never an unstyled anchor.
