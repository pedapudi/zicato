...
export async function render(host, ctx) {
  ...
  gatedSwap(host, digest, () => {
    ...
    const bumpsCard = el('div', { class: 'd-panel d-fit' });
    ...
    // ---- jumps to the two new views ----
    nodes.push(el('div', { class: 'e-jump-row', style: 'display:flex;gap:10px;flex-wrap:wrap;margin:-6px 0 0;' }, [
      el('a', { class: 'h-link-btn', href: ctx.href('mutations', {}) }, ['Mutation sites × generations ', el('span', { 'aria-hidden': 'true', text: '→' })]),
      el('a', { class: 'h-link-btn', href: ctx.href('report', { epochId }) }, ['Epoch analysis report ', el('span', { 'aria-hidden': 'true', text: '→' })]),
    ]));

    // ---- board entries × generations heatmap ----
    const rows = [...entryIds].sort().map((id) => ({ id, label: id }));
    const cols = gens.map((g) => ({ id: g.id, label: g.id }));
    const hmCard = el('div', { class: 'd-panel d-fit' });
