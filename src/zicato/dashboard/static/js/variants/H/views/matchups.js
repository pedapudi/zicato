// variants/E/views/matchups.js — MATCH-UPS across tournament styles.
//
...
// Atlas's match-ups screen shows, from D's data-viz:
//   1. the gauntlet LADDER — a non-colliding bumps chart;
//   2. per-round PAIRED SLOPEGRAPHS — champion loss → challenger loss for
//      every board entry, de-collided + jittered so coincident values stay
//      legible (the heart of a single round); click a line → its run;
//   3. ALTERNATIVE styles (illustrative overlays on the SAME candidate set,
//      a different topology each): a single-elim bracket tree, a round-robin
//      matrix, and racing / successive-halving race lanes — clearly labelled
//      illustrative, because only the gauntlet has real per-round data.
//
...
export async function render(host, ctx) {
  ...
  if (!ep || ep.epoch_id == null) {
    gatedSwap(host, 'no-epoch', () => [el('h1', { class: 'd-h1', text: 'Match-ups' }), empty('No current epoch.')]);
    return;
  }
  ...
  gatedSwap(host, digest, () => {
    const out = [];
    out.push(el('div', { class: 'e-pagehead' }, [
      el('h1', { class: 'd-h1', text: 'Match-ups across tournament styles' }),
      el('p', { class: 'd-lede', text: 'zicato runs a king-of-the-hill gauntlet — one champion, one challenger per round, paired per-board duels. Below: the real gauntlet, then the same candidates under other structures.' }),
    ]));

    // 1. gauntlet ladder (fit-to-width, no viewport)
    const ladder = el('div', { class: 'd-panel d-fit' });
    ladder.appendChild(svg.bumps({ width: 720, height: 190, nodes, onClick: (n) => ctx.navigate('candidate', { gen: n.id }) }));
    ladder.appendChild(el('div', { class: 'd-legend' }, [
      ...
      el('span', { class: 'd-faint', text: 'real gauntlet · click a node → candidate' }),
    ]));
    ...
    if (anyGrid) slopeCard.appendChild(el('div', { class: 'd-legend' }, [
