...
function renderBracket(st, ctx, epochId, structure) {
  ...
  const standings = standingsTable(st, ctx, epochId);
...
function renderRacing(st, ctx, epochId) {
  ...
  card.appendChild(rungs.length
    ? svg.racingLadder({ rungs, onCompetitor: (gen) => { if (gen) ctx.navigate('candidate', { epochId, gen }); } })
    : empty('No rungs evaluated yet.'));
  if (rungs.length) {
    card.appendChild(el('p', { class: 'dn-faint', style: 'font-size:11px;margin:8px 0 0;', text: 'each rung evaluates the survivors on a fraction of the board, then cuts the worst · ✕ = cut · ↑ = survives · click a competitor → open' }));
  }
  nodes.push(section('Racing ladder · rungs, board fractions, cuts', card));
  const standings = standingsTable(st, ctx, epochId);
  if (standings) nodes.push(section('Standings', standings));
  return nodes;
}
...
function standingsTable(st, ctx, epochId) {
...
function statusPill(status) {
  ...
  // reads in every theme: champion→promoted, eliminated→rejected, alive→deferred.
