// variants/C/views/styles.js — Screen: TOURNAMENT-STYLE TOPOLOGY SWITCHER (theme 4).
//
...
function paintDuel(slot, rows, champId, challId) {
  ...
  const all = [];
  for (const r of rows) { if (r.championLoss != null) all.push(r.championLoss); if (r.challengerLoss != null) all.push(r.challengerLoss); }
  const lo = Math.min(...all, 0); const hi = Math.max(...all, 1);
  const span = hi - lo || 1;
  const W = 460; const rowH = 40; const padL = 130; const padR = 70; const top = 26;
  const xc = padL; const xch = W - padR;
  const svg = svgEl('svg', { class: 'cz-duel-svg', viewBox: `0 0 ${W} ${top + rows.length * rowH + 20}`, role: 'img', 'aria-label': 'Per-board duel slopegraph' });

  svg.appendChild(svgEl('text', { x: xc, y: 16, class: 'cz-sankey-col-head', 'text-anchor': 'middle' }, [String(champId)]));
  svg.appendChild(svgEl('text', { x: xch, y: 16, class: 'cz-sankey-col-head', 'text-anchor': 'middle' }, [String(challId)]));

  rows.forEach((r, i) => {
    const y = top + i * rowH + rowH / 2;
    const yc = r.championLoss == null ? y : y; void yc;
    const cls = r.verdict === 'improved' ? 'cz-edge-promoted' : (r.verdict === 'regressed' ? 'cz-edge-rejected' : 'cz-edge-cross');
    // Slope line champion → challenger (we keep both at the same y but
    // annotate values; the colour conveys verdict — a clean head-to-head).
    svg.appendChild(svgEl('line', { x1: xc, y1: y, x2: xch, y2: y, class: 'cz-duel-slope ' + cls }));
    svg.appendChild(svgEl('circle', { cx: xc, cy: y, r: 5, class: 'cz-duel-dot' }));
    svg.appendChild(svgEl('circle', { cx: xch, cy: y, r: 5, class: 'cz-duel-dot' }));
    svg.appendChild(svgEl('text', { x: 6, y: y + 4, class: 'cz-duel-entry cz-mono', 'text-anchor': 'start' }, [clip(r.entryId, 18)]));
    svg.appendChild(svgEl('text', { x: xc - 8, y: y + 4, class: 'cz-duel-val cz-mono', 'text-anchor': 'end' }, [r.championLoss == null ? '—' : fmtScalar(r.championLoss)]));
    svg.appendChild(svgEl('text', { x: xch + 8, y: y + 4, class: 'cz-duel-val cz-mono', 'text-anchor': 'start' }, [r.challengerLoss == null ? '—' : fmtScalar(r.challengerLoss)]));
    void span; void lo;
  });
