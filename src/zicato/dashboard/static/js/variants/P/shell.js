// variants/P/shell.js — the Console III shell: a data-model TREE sidebar +
...
//
// Theme + typeface are CSS-only swaps (data-p-theme / data-p-type on the root).
...
export function applyTheme(theme, rootEl) {
  ...
  if (root) root.setAttribute('data-p-theme', t);
  persistColor(t);
...
export function applyTypeface(typeface, rootEl) {
  ...
  const root = rootEl || _root;
  if (root) root.setAttribute('data-p-type', t);
  persistType(t);
...
export function mountShell(root) {
  ...
  root.setAttribute('data-variant', 'P');
  root.setAttribute('data-p-theme', readColor());
  root.setAttribute('data-p-type', readType());
  ...
  const topbar = el('header', { class: 'dp-topbar' }, [
    el('div', { class: 'dp-brand' }, [
      ...
      el('span', { class: 'dp-brand-variant', text: 'console iii' }),
    ]),
    _crumbHost,
  ...
  if (!String(location.hash).startsWith('#/P')) location.hash = '#/P/';
  else dispatch();
}
...
async function dispatch() {
  ...
  try {
    ...
    console.error('variant-P render error', err);
  }
}
