// variants/P/views/candidate.js — CANDIDATE (one generation), convergence III.
//
// Console III's candidate screen pairs the compact lifecycle DAG with the
// per-board scoring dot-plot, and adds the round-5 fixes:
//   * fix #1 — the STACKED, non-overlapping PROMOTE GATE lives on THIS page
//     (N lacked it): decision header · rules ladder · scalar-components block.
//   * fix #2 — the lifecycle "patch" node is clickable → this candidate's
//     SIDE-BY-SIDE diff (views/diff.js).
...
export async function render(host, ctx, params) {
  ...
  // fix #1 — the gate(s) for this candidate. As a challenger: its own round's
  // gate (parent → gen). As a champion: each defended round's gate.
  const gateKeys = [];
  ...
  const digest = JSON.stringify({
    ...
    delta: svg.isNum(primaryDelta) ? primaryDelta.toFixed(3) : null,
    mpts,
    ...
    matchups: mine.map((m) => [m.champion, m.challenger, m.decision, svg.isNum(m.delta_scalar) ? m.delta_scalar.toFixed(2) : null]),
    gates: gates.map((g, i) => g && Array.isArray(g.rules)
      ? [gateSpecs[i].champ, gateSpecs[i].chall, gateSpecs[i].role, g.decision, svg.isNum(g.delta_scalar) ? g.delta_scalar.toFixed(3) : null, g.rules.map((r) => [r.id, r.status, r.fired])]
      : null),
  ...
  gatedSwap(host, digest, () => {
    ...
    nodes.push(el('div', { class: 'dn-pagehead' }, [
      el('h1', { class: 'dn-h1' }, [(node.promoted ? '♛ ' : '') + 'Candidate ' + genId]),
      el('p', { class: 'dn-lede', text: baseline ? 'The seed candidate (no parent) — it defines the loss floor for the epoch.' : `Born from ${node.parent} by a patch; faced the board; met the champion at the gate.` }),
    ]));
    ...
    nodes.push(el('div', { class: 'dn-panel dn-row' }, [
      ...
      el('div', { class: 'dn-stat' }, [verdictPill(baseline ? 'baseline' : decision)]),
    ]));
    ...
    dagCard.appendChild(lifecycleDag({
      ...
      deltaScalar: primaryDelta, patchPoints: mpts, entries,
      width: 900, height: Math.max(300, 120 + entries.length * 34),
      onEntry: (eid) => ctx.navigate('candidate', { epochId, gen: genId, entry: eid }),
    ...
    if (entries.length) {
      ...
      scoreCard.appendChild(svg.valueDotPlot({
        ...
        onClick: (it) => ctx.navigate('candidate', { epochId, gen: genId, entry: it.id }),
      }));
    ...
    nodes.push(section('Match-ups · every round this candidate was in', allMatchupsPanel(mine, genId, championId, ctx, epochId)));

    // ---- fix #1: the STACKED promote gate(s) on the candidate page ----
    if (gates.some((g) => g && Array.isArray(g.rules))) {
      gateSpecs.forEach((k, i) => {
        const g = gates[i];
        if (!g || !Array.isArray(g.rules)) return;
        nodes.push(section(`Promote gate · ${k.champ} → ${k.chall} (${k.role})`, gatePanel(g, k.champ, k.chall)));
      });
    } else if (!baseline) {
      nodes.push(section('Promote gate', el('div', { class: 'dn-panel' }, [empty('No gate decomposition recorded for this candidate’s round.')])));
    }
    return nodes;
  });
}

// fix #3 — every matchup the candidate was in, both roles, with a drill into
// the paired per-board duel (the board view, inline transcript).
function allMatchupsPanel(mine, genId, championId, ctx, epochId) {
  ...
  for (const m of mine) {
    ...
    tbody.appendChild(el('tr', null, [
      el('td', null, [el('a', { class: 'dn-mono dn-mtx-genlink', href: ctx.href('candidate', { epochId, gen: asChamp ? m.challenger : m.champion }), text: `${m.champion} → ${m.challenger}` })]),
  ...
  card.appendChild(el('p', { class: 'dn-faint', style: 'font-size:11px;margin:10px 0 0;', text: genId === championId
    ? `as champion, ${genId} defended ${mine.length} round${mine.length === 1 ? '' : 's'} · click a round → the opposing candidate`
    : `${mine.length} round${mine.length === 1 ? '' : 's'} · click a round → the opposing candidate` }));
  return card;
}

// fix #1 — the stacked, non-overlapping gate panel (ported from N's match-ups):
