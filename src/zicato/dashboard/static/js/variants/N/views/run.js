// variants/N/views/run.js — RUN DETAIL: the transcript.
//
...
export async function render(host, ctx, params) {
  ...
  if (!host.firstChild) host.appendChild(el('p', { class: 'dn-empty', text: 'Loading run…' }));

  if (!genId || !entryId) {
    gatedSwap(host, 'no-run', () => [
      el('h1', { class: 'dn-h1', text: 'Run' }),
  ...
  gatedSwap(host, digest, () => {
    ...
    nodes.push(el('div', { class: 'dn-pagehead' }, [
      el('h1', { class: 'dn-h1', text: `Run · ${entryId}` }),
      el('p', { class: 'dn-lede' }, [
        `The reconstructed conversation for ${genId} on this board entry`,
        runId ? el('span', { class: 'dn-faint dn-mono', text: '  ' + runId.slice(0, 12) + '…' }) : null,
      ].filter(Boolean)),
    ]));
...
function transcriptPanel(conv, runId, turns, anns) {
  ...
  const scroller = el('div', { class: 'dn-transcript' });
