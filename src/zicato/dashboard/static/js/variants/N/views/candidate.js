// variants/N/views/candidate.js — CANDIDATE (one generation).
//
...
export async function render(host, ctx, params) {
  ...
  gatedSwap(host, digest, () => {
    ...
    dagCard.appendChild(el('p', { class: 'dn-faint', style: 'font-size:11px;margin:8px 0 0;', text: 'parent → patch → board (one node per entry, colour = pass/fail/timeout) → Σ → gate → terminal · click a board node → its drill-down' }));
...
function entryDrilldown(ctx, genId, entryId, row, exps, judges) {
  ...
  card.appendChild(el('div', { style: 'margin-top:14px;' }, [
    el('a', { class: 'dn-linkbtn', href: ctx.href('run', { gen: genId, entry: entryId }), text: 'Open the full run transcript →' }),
