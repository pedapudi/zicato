// variants/N/views/epoch.js — EPOCH: the dense data substrate of one epoch.
//
...
export async function render(host, ctx) {
  ...
  gatedSwap(host, digest, () => {
    ...
    // links to the NEW convergence-II views
    nodes.push(el('div', { class: 'dn-quicklinks' }, [
      el('a', { class: 'dn-linkbtn', href: ctx.href('mutations', {}), text: 'Mutation surface + diff →' }),
      el('a', { class: 'dn-linkbtn', href: ctx.href('matchups', {}), text: 'Match-ups + promote gate →' }),
    ...
    if (rows.length && cols.length) {
      ...
      hmCard.appendChild(el('p', { class: 'dn-faint', style: 'font-size:11px;margin:8px 0 0;', text: 'cell = drift loss for one board entry in one generation · denser ink = more drift · click a row → that board across every candidate' }));
    } else {
...
function boardTrellis(board, gens, rowByGenEntry, domain, ctx) {
  ...
  for (const b of sorted) {
    ...
    // FIX #7: a trellis card routes to the PER-BOARD cross-candidate view by
    // its entry id — not to an arbitrary candidate (the old "dump on v2" bug).
  ...
  card.appendChild(legend([['spine', 'one bar per candidate · drift loss (shared scale)'], ['dotact', 'pass'], ['dotpred-bad', 'fail']], '⏱ timeout · click a board → that entry across every candidate'));
  return card;
}
