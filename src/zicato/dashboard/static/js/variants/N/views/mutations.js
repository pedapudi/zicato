// variants/N/views/mutations.js — ONE cohesive visual: the mutation surface
...
export async function render(host, ctx, params) {
  ...
  gatedSwap(host, digest, () => {
    ...
    const combined = el('div', { class: 'dn-mut-combined' }, [
      el('div', { class: 'dn-panel dn-mut-matrix', style: 'overflow-x:auto;' }, [matrixTable(sites, gens, patchedBySite, pinned, ctx)]),
      el('div', { class: 'dn-panel dn-mut-detail' }, [detailPane(pinnedSite, baselineStr, detail, patchesByGen, ctx)]),
    ]);
...
function matrixTable(sites, gens, patchedBySite, pinned, ctx) {
  ...
  for (const g of gens) hr.appendChild(el('th', { class: 'dn-mtx-gen' }, [
    el('a', { class: 'dn-mtx-genlink', href: ctx.href('candidate', { gen: g }), text: g }),
  ]));
  ...
  for (const s of sites) {
    ...
    tr.appendChild(el('th', { class: 'dn-mtx-site', scope: 'row' }, [
      el('a', { class: 'dn-mtx-sitelink', href: ctx.href('mutations', { mutId: s.mutation_id }), title: s.mutation_id }, [
    ...
    for (const g of gens) {
      ...
      if (on) {
        ...
        td.appendChild(el('a', { class: 'dn-mtx-celllink', href: ctx.href('mutations', { mutId: s.mutation_id }), title: `${g} patched ${s.mutation_id}` }, [dot]));
      } else {
...
function detailPane(site, baselineStr, detail, patchesByGen, ctx) {
  const pane = el('div');
  if (!site) {
  ...
  for (const g of touched) {
    ...
    any = true;
    pane.appendChild(genDiffBlock(g, patch, baselineStr == null ? '' : baselineStr, newStr, site, ctx));
  }
  if (!any) pane.appendChild(el('p', { class: 'dn-empty', text: 'No generation patched this site (or the patch payloads are unavailable).' }));
  return pane;
}

function genDiffBlock(gen, patch, baselineStr, newStr, site, ctx) {
  ...
  block.appendChild(el('div', { class: 'dn-patch-head' }, [
    el('a', { class: 'dn-mtx-genlink', href: ctx.href('candidate', { gen }), text: gen }),
