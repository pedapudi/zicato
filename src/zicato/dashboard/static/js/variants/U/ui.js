// variants/U/ui.js — shared chrome helpers for Variant U ("Atlas V").
//
// Self-contained for Variant U. Small, pure builders the comparison-first
...
//
// U defaults to **solarized-light** colour + **Sans** typeface — the calm,
// airy take. The marks (svg.js / dag.js, carried from the console base) emit
// `dn-*` / `ezn-*` classes that read the shared `--v2-*` tokens; U's own chrome
// uses `vu-*`. Both pickers persist under U-scoped localStorage keys. No data
// fetching here.
...
export function gatedSwap(host, digest, build) {
  ...
  if (host.getAttribute('data-u-digest') === next && host.firstChild) return false;
  clearChildren(host);
  const built = build();
  const nodes = Array.isArray(built) ? built : [built];
  for (const n of nodes) { if (n) host.appendChild(n); }
  host.setAttribute('data-u-digest', next);
  return true;
}

// ---- colour themes (solarized-light is U's default) -----------------
...
const COLOR_KEY = 'zicato.U.theme';
...
// ---- typeface themes (Sans is U's default) --------------------------
//
// Each maps to a `[data-u-type]` value the stylesheet keys on (swapping the
// --u-font-* / --v2-sans / --v2-mono custom properties). All Open-Sans-based
// for the body; the distinction is the heading / data voice:
//   Sans      — Open Sans throughout (tabular figures for data) — U's default.
...
const TYPE_KEY = 'zicato.U.typeface';
