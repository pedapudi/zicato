// variants/U/views/env.js — ENVIRONMENT overview (the all-epochs root).
//
...
export async function render(host, ctx) {
  ...
  gatedSwap(host, digest, () => {
    ...
    const grid = el('div', { class: 'vu-epoch-grid' });
    for (const e of list) {
      const card = el('a', {
        class: 'vu-epoch-card' + (e.epoch_id === currentId ? ' vu-epoch-current' : ''),
      ...
      card.appendChild(el('div', { class: 'vu-epoch-head' }, [
        el('span', { class: 'vu-epoch-id', text: e.epoch_id }),
        el('span', { class: 'dn-chip ' + (e.closed ? 'dn-chip-closed' : 'dn-chip-open'), text: e.epoch_id === currentId ? 'current' : (e.closed ? 'closed' : 'open') }),
      ]));
      if (e.goal) card.appendChild(el('p', { class: 'vu-epoch-goal', text: e.goal }));
