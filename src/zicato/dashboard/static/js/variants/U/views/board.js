// variants/U/views/board.js — PER-BOARD cross-candidate view + the SIGNATURE
// INLINE side-by-side transcript (fix #5 — the centrepiece).
//
// First-class from the tree's Boards group (fix #4). For ONE board entry it
// shows how EVERY candidate performed (sorted comparative dot-plot + a tabular
// breakdown), the board TRELLIS small-multiple for that entry (fix #6 — the
// trellis lives in the Boards view, never on the epoch overview), and — the
// signature — TWO candidates' transcripts on that board shown SIDE BY SIDE,
// INLINE in the detail pane. Selecting runs does NOT navigate away: it sets the
// `runs` route param (URL-encoded) and the two transcripts fill two
// independently-scrollable columns within this same view.
//
// Data: /api/epoch, /api/lineage, /api/generation/{e}/{g}/per-entry pivoted by
// entry_id, /api/matchup-grid/… (champion vs challenger run ids),
// /api/conversation/{run_id} per candidate.
...
export async function render(host, ctx, route) {
  ...
  if (!entryId) {
    gatedSwap(host, 'no-entry', () => [el('h1', { class: 'dn-h1', text: 'Board entry' }),
      empty('No board entry selected — open one from the tree’s Boards group or the epoch heatmap.')]);
    return;
  }
  ...
  // resolve the two transcript SIDES (fix #5). Default to champion vs the first
  // challenger that ran when no explicit pair is in the route.
  const runSel = (route.runs && route.runs.length) ? route.runs.slice(0, 2) : null;
  ...
  const digest = JSON.stringify({
    ...
    genA, genB,
    convA: convDigest(convA), convB: convDigest(convB),
  });
  ...
  gatedSwap(host, digest, () => {
    ...
    nodes.push(el('div', { class: 'dn-pagehead' }, [
      ...
      el('p', { class: 'dn-lede', text: 'How every candidate performed on this one entry — then read two candidates’ transcripts on it side by side, inline below.' }),
    ]));
    ...
    if (items.length) {
      scoreCard.appendChild(svg.valueDotPlot({
        ...
        // a candidate → load its transcript as side A of the inline comparison.
    ...
    // the board TRELLIS small-multiple for THIS entry (fix #6 — trellis here).
    nodes.push(section('Trellis · drift loss across candidates (this entry)', trellisCell(def, entryId, rows, champLoss)));
    ...
    for (const r of rows.slice().sort((a, b) => (svg.isNum(b.loss) ? b.loss : -1) - (svg.isNum(a.loss) ? a.loss : -1))) {
      const setRun = el('a', { class: 'dn-linkbtn dn-board-run',
        href: ctx.href('board', { epochId, entry: entryId }, { runs: [r.gen, genB && genB !== r.gen ? genB : (genA !== r.gen ? genA : null)].filter(Boolean) }),
        text: 'show transcript →',
        onclick: (ev) => { ev.preventDefault(); ctx.navigate('board', { epochId, entry: entryId }, { runs: [r.gen, genB && genB !== r.gen ? genB : (genA !== r.gen ? genA : null)].filter(Boolean) }); } });
    ...
    // ── the SIGNATURE: inline side-by-side transcripts (fix #5) ──
    const cmpBar = el('div', { class: 'vu-cmp-bar' }, [
      comparePicker({
    ...
    const transWrap = el('div', { class: 'vu-transcripts' }, [cmpBar, split]);
    nodes.push(section('Transcripts · side by side, inline · ' + entryId, transWrap));
    return nodes;
  });
}

// Paint one candidate's transcript into an independently-scrollable column.
function transcriptInto(host, conv, row) {
  ...
  const scroller = el('div', { class: 'dn-transcript vu-transcript' });
...
function trellisCell(def, entryId, rows, champLoss) {
  ...
  card.appendChild(el('div', { class: 'dn-legend' }, [
    ...
    el('span', null, [el('i', { class: 'dotact' }), 'pass']),
    el('span', null, [el('i', { class: 'dotpred', style: 'border-color:var(--v2-bad);' }), 'fail']),
    el('span', { class: 'dn-faint', text: '⏱ timeout · the trellis lives here in the Boards view (fix #6)' }),
  ]));
  return card;
}
