// variants/U/router.js — Variant U ("Atlas V") tree-node hash router.
//
// U replaces top-tab navigation with a persistent DATA-MODEL TREE. The hash
...
//   #/U/e/<epochId>                            → epoch overview (heatmap)
//   #/U/e/<epochId>/gen/<gen>                  → candidate (lifecycle · gate · matchups)
//   #/U/e/<epochId>/gen/<gen>/patch            → candidate, patch diff opened
//   #/U/e/<epochId>/gen/<gen>/entry/<entry>    → candidate, one entry drilled
//   #/U/e/<epochId>/board/<entry>              → per-board cross-candidate view
//   #/U/e/<epochId>/mut[/<mutId>]              → mutation surface (+ side-by-side diff)
//   #/U/e/<epochId>/pub                        → epoch publication (ACM)
//
...
export function parseRoute(hash) {
  ...
  if (!path.startsWith('/U')) return { view: 'env', params: {}, cmp: null, runs: null };
  const parts = path.replace(/^\/U\/?/, '').split('/').filter(Boolean).map(dec);
...
// The destination ONE step UP the selection hierarchy — drives the FIXED
// back/up control (Round-6 NEW #3). Returns { view, params, opts } or null when
// already at the environment root. The shell wires the control to `navigate()`
// this, so the destination always renders into the MAIN detail pane (never the
// sidebar — Q's bug). Drilled sub-nodes (patch / entry / a chosen comparison /
// a board run pair) step up WITHIN the candidate/board before leaving it.
export function parentRoute(route) {
  const p = route.params || {};
  switch (route.view) {
    case 'candidate': {
      // a drilled sub-node (patch / entry) or an active comparison steps back to
      // the bare candidate first.
      if (p.sub === 'entry' || p.sub === 'patch') return { view: 'candidate', params: { epochId: p.epochId, gen: p.gen }, opts: route.cmp ? { cmp: route.cmp } : undefined };
      if (route.cmp) return { view: 'candidate', params: { epochId: p.epochId, gen: p.gen } };
      return { view: 'epoch', params: { epochId: p.epochId } };
    }
    case 'board': {
      // an inline transcript pair steps back to the bare board first.
      if (route.runs && route.runs.length) return { view: 'board', params: { epochId: p.epochId, entry: p.entry } };
      return { view: 'epoch', params: { epochId: p.epochId } };
    }
    case 'mutations':
    ...
    default:
      return null; // already at environment root
  }
}
