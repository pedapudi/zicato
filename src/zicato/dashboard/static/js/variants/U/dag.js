// variants/U/dag.js — the compact candidate-lifecycle DAG (Atlas V).
//
// Self-contained, deliberately SMALL: a single static SVG (NO pan/zoom) reading
// one candidate's life left-to-right — PARENT ▶ PATCH ▶ [board fan] ▶ Σ ▶ GATE ▶
// TERMINAL. Mark classes are `ezn-*`, styled (scoped under the variant root) by
// css/variants/U/atlasv.css. The PATCH node accepts an `onPatch` handler so it
// is clickable → the candidate's side-by-side patch diff (fix #2).
