// js/tournament_model.js — the tournament STRUCTURE models.
//
// Pure derivations over the served tournament payloads (/api/active-tournament,
// /api/tournament-structure, /api/epoch/{id}/racing-field, /api/active-runs and
// the heartbeat): the normalised structure, the one resolver every page reads a
// non-gauntlet structure through, the per-structure models the figures draw,
// the unified live model with its match blocks, the digests the gated swaps
// compare, and the epoch-scoped liveness helpers. Nothing here builds DOM: the
// structure page renders these models in views/structure.js, the live hero in
// live.js.

import * as svg from './svg.js';
import { overrideDigest, pendingOverrideDigest, ratingTripleDigest } from './ui.js';
...
export function diversityMembership(st) {
...
export function splitBand(rounds, keep) {
  const out = [];
