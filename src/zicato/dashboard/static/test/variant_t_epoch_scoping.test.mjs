...
// CURRENT epoch), then gated the crown on `isContractEpoch`. Every promoted
// generation in a CLOSED epoch therefore fell to `formerChampion`, so that
// epoch's own reigning champion read "former champion" and nothing there was
// crowned. Each epoch node now reads its OWN `?epoch=`-scoped pointer.
// ====================================================================
...
test('tree model (cross-epoch): EACH epoch crowns its OWN champion — a closed epoch does not read all-former', async () => {
  freshState();
  ...
  // e0 (CLOSED, non-current): v4 REIGNS, v0 is the former champion.
